...
function gh_request(string $url,string $token=''): ?string {
    if(!function_exists('curl_init'))throw new RuntimeException('PHP cURL extension is required');$token=$token!==''?$token:(string)cfg()['gh_token'];$ch=curl_init($url);$headers=['User-Agent: WebConsole-Pro/1.2.2','Accept: application/vnd.github+json'];if($token!=='')$headers[]='Authorization: Bearer '.$token;
