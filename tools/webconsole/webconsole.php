...
function cfg(): array {
    ...
    $d=['pass_hash'=>'','created'=>date('c'),'theme'=>'dark','layout'=>'classic','density'=>'comfortable','fs_start'=>is_dir('/var/www')?'/var/www':'/','fs_roots'=>['/'],'session_minutes'=>180,'allowed_ips'=>'','gh_token'=>'','gh_repo'=>'','gh_branch'=>'backups','git_name'=>'webconsole','git_email'=>'webconsole@localhost','split_mb'=>80,'tmux_width'=>120,'tmux_height'=>34];
...
function gh_request(string $url,string $token=''): ?string {
    if(!function_exists('curl_init'))throw new RuntimeException('PHP cURL extension is required');$token=$token!==''?$token:(string)cfg()['gh_token'];$ch=curl_init($url);$headers=['User-Agent: WebConsole-Pro/1.2.0','Accept: application/vnd.github+json'];if($token!=='')$headers[]='Authorization: Bearer '.$token;
...
function handle_api() {
    ...
    case 'settings.get':
        $c=cfg();jout(true,['theme'=>$c['theme'],'layout'=>$c['layout'],'density'=>$c['density'],'fs_start'=>$c['fs_start'],'session_minutes'=>$c['session_minutes'],'allowed_ips'=>$c['allowed_ips'],'created'=>$c['created'],'noexec'=>$GLOBALS['__NOEXEC']]);
    case 'settings.save':
        $new=[];foreach(['theme'=>['dark','light','forest','ocean','amber'],'layout'=>['classic','studio','focus'],'density'=>['comfortable','compact']] as $key=>$allowed){if(isset($in[$key])){if(!in_array($in[$key],$allowed,true))jout(false,null,'Invalid appearance option: '.$key);$new[$key]=$in[$key];}}if(isset($in['fs_start']))$new['fs_start']=safe_path((string)$in['fs_start']);if(isset($in['session_minutes']))$new['session_minutes']=max(10,min(1440,(int)$in['session_minutes']));if(isset($in['allowed_ips']))$new['allowed_ips']=trim((string)$in['allowed_ips']);cfg_save($new);jout(true);
