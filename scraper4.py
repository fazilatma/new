...
class Fetcher:
    ...
    def get(self, url: str, *, referer: str = "", accept_json: bool = False, engine: str = "requests") -> FetchResult:
        ...
        for attempt in range(3):
            try:
                if engine in {"beautifulsoup", "beautifulsoup4", "bs4"}:
                    try:
                        import bs4
                    except ImportError as exc:
                        raise FetchError("کتابخانه beautifulsoup4 نصب نیست") from exc
                    response = self.session.get(request_url, headers=headers, timeout=self.timeout, allow_redirects=True, verify=self.verify, stream=True)
...
def create_soup(markup: str, default_parser: str = "lxml") -> BeautifulSoup:
    """Safely construct BeautifulSoup object falling back to available parsers."""
    for parser in (default_parser, "lxml", "html5lib", "html.parser"):
        try:
...
HTTP_ENGINE_ORDER = ("requests", "beautifulsoup", "httpx", "curl_cffi", "cloudscraper")
KNOWN_ENGINES = HTTP_ENGINE_ORDER + ("playwright", "selenium", "undetected_chromedriver")
...
def picker_http_fetch(url: str, fetcher: "Fetcher", errors: list[str]) -> Any:
    relay_dead = False
    for engine in ("requests", "beautifulsoup", "httpx", "cloudscraper", "curl_cffi"):
...
def health():
    engines_status = {
        "beautifulsoup4": fetch_engine_installed("beautifulsoup"),
        "lxml": fetch_engine_installed("lxml"),
        "html5lib": fetch_engine_installed("html5lib"),
        "requests": True,
        "httpx": fetch_engine_installed("httpx"),
        "cloudscraper": fetch_engine_installed("cloudscraper"),
        "curl_cffi": fetch_engine_installed("curl_cffi"),
        "playwright": fetch_engine_installed("playwright"),
        "selenium": fetch_engine_installed("selenium"),
    }
...
def api_deploy_dependencies():
    ...
    if VPS_MODE:
        packages = ["flask", "requests", "beautifulsoup4", "lxml", "html5lib", "playwright", "basalam-sdk", "cloudscraper", "curl_cffi", "httpx", "selenium", "playwright-stealth", "undetected-chromedriver"]
