...
نصب روی PythonAnywhere (داخل Bash Console):
    cd ~/scraper4
    curl -fsSL https://raw.githubusercontent.com/fazilatma/new/arena/01a0bd3f-new/python-scraper4/setup_deployer4.sh -o setup_deployer4.sh
...
DEFAULT_BRANCHES = [
    b for b in os.environ.get("DEPLOYER_BRANCHES", "arena/01a0bd3f-new").split(",")
    if b.strip()
] or ["arena/01a0bd3f-new"]
...
def _git_dir() -> str:
    ...
    if env:
        return env
    # Same writability fallback as scraper4._scraper_git_dir: /opt is not
    # writable for non-root installs.
    legacy = "/opt/amphp"
    try:
        ...
        pass
    fallback = os.path.join(BASE_DIR, ".git-cache")
    try:
        ...
        pass
    fallback = os.path.join(tempfile.gettempdir(), "scraper4-git-cache")
    os.makedirs(fallback, exist_ok=True)
    return fallback
