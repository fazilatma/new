...
CHANGELOG = [
    ...
    {"version":"10.237","date":"2026-09-23","title":"وب‌کنسول ۱.۶.۵ — دکمهٔ نصب/به‌روزرسانی خودش پلی‌رایت و کرومیوم را نصب می‌کند","items":["ابزار tools/webconsole-patch-1.6.5.php اضافه شد؛ وب‌کنسول شما را از 1.6.4 به 1.6.5 ارتقا می‌دهد: php patch_webconsole_1.6.5.php webconsole.php","بعد از هر استقرار پروژهٔ پایتون، مرحلهٔ غیرمُرد browser اجرا می‌شود: نصب playwright با pip، دانلود کرومیوم از CDN رسمی و در شکست از آینه‌های ایران (نسخهٔ دقیق از dry-run خوانده می‌شود)، آزمایش بالا آمدن مرورگر و در نهایت کرومیوم سیستمی apt","دکمه‌های نصب مؤلفه‌ها (browser_deps / python_scrapers / all) در داشبورد هم همین بوت‌استرپ مرورگر را اجرا می‌کنند","فهرست کتابخانه‌های پیش‌فرض پایتون وب‌کنسول با موتورهای کامل تکمیل شد: gunicorn curl_cffi cloudscraper httpx aiohttp undetected-chromedriver playwright selenium","پچر ایمن است: پیش از نوشتن، هر ۶ لنگر را بررسی می‌کند، نسخهٔ پشتیبان زمان‌دار می‌سازد، خروجی را با php -l تأیید و در خطا خودکار برمی‌گرداند؛ اجرای دوباره‌اش کاری نمی‌کند"]},
    {"version":"10.236","date":"2026-09-23","title":"نصب کرومیوم داخل خود شل نصب‌کننده — بدون فایل جداگانه","items":["منطق کامل نصب Chromium از آینه (npmmirror/huaweicloud/…) عیناً داخل run_scraper4.sh قرار گرفت: نسخهٔ دقیق خواسته‌شدهٔ پلی‌رایت از install --dry-run خوانده، بسته‌های Chrome-for-Testing از آینه‌ها دانلود و با ساختار و نشان INSTALLATION_COMPLETE در کش باز می‌شوند، کتابخانه‌های سیستمی نصب و بالا آمدن مرورگر آزمایش می‌شود — دیگر هیچ فراخوانی به فایل جداگانهٔ install_chromium_mirror.sh وجود ندارد","همین منطق به‌صورت درون‌خطی داخل نصب‌کنندهٔ VPS (tools/vps-live/install_scraper4_vps.sh) هم قرار گرفت","ترتیب نصب مرورگر در run_scraper4.sh: اول دانلود رسمی playwright install chromium، در شکست آینه‌های درون‌خطی، در شکست کرومیوم سیستمی (apt) — و در هر حالت پیام روشن نهایی","فایل tools/install_chromium_mirror.sh همچنان برای اجرای دستی مستقل و ارجاع‌های راهنمای برنامه باقی است؛ فقط نصب‌کننده‌های خودکار دیگر به آن وابسته نیستند"]},
    ...
    {"version":"10.193","date":"2026-09-20","title":"تست صفحه‌بندی در عیب‌یابی استخراج","items":["عیب‌یابی استخراج حالا مانند تست سرعت، صفحه‌بندی پروفایل را با رفتن واقعی به صفحه ۲ (و ۳) می‌آزماید","اگر الگوی مسیر/~page~/query اشتباه باشد، URL تکراری بماند، next پیدا نشود یا صفحه خالی برگردد، مرحله pagination با خطای واضح «خطای صفحه‌بندی در صفحه N» گزارش می‌شود","پشتیبانی از auto، تیلدا ایمالز و انواع path/query/full/next با گزارش تعداد محصول هر صفحه"]},
    {"version":"10.192","date":"2026-09-20","title":"تست سرعت ۳ صفحه‌ای با صفحه‌بندی واقعی","items":["تست سرعت حالا واقعاً ۳ صفحه را با همان صفحه‌بندی پروفایل (query/path/~page~/full/next) می‌آزماید نه فقط صفحهٔ اول","اگر صفحهٔ ۲ یا ۳ با الگوی فعلی ساخته نشود، خالی برگردد یا next پیدا نشود، خطای واضح «خطای صفحه‌بندی در صفحه N: …» گزارش می‌شود","پشتیبانی کامل از تیلدا ~page~ ایمالز و تشخیص خودکار در تست؛ pagesScanned و products مجموع ۳ صفحه گزارش می‌شود"]},
    {"version":"10.191","date":"2026-09-20","title":"رفع صفحه‌بندی الگوی مسیر با تیلدا ~page~ برای ایمالز","items":["الگوی مسیر حالا افزون بر /page/{page}/، الگوهای تیلدا مانند ~page~{page} (ایمالز) را هم بدون افزودن / اضافه پشتیبانی می‌کند و پسوند قبلی ~page~2 را درست حذف می‌کند","تشخیص خودکار صفحه‌بندی لینک‌های ~page~2 / ~Page~2 را هم به‌عنوان path شناسایی می‌کند تا پروفایل‌های ایمالز نیاز به تنظیم دستی نداشته باشند","اعتبارسنجی دستی مقدار صفحه‌بندی با تیلدا تست شد: https://emalls.ir/...~Category~31424 → ~page~2 بدون اسلش اضافی"]},
    {"version":"10.190","date":"2026-09-20","title":"متناسب‌سازی بخش نسخه و دیپلویر برای پایتون","items":["بخش «نسخهٔ کد و دیپلویر» در منوی همبرگر کاملاً برای پایتون بازنویسی شد: عنوان، راهنما و دستورها حالا از git و systemd/gunicorn می‌گویند نه Cloudflare/Render","کارت «تنظیمات به‌روزرسانی خودکار» به خود اسکریپر اضافه شد — فعال/غیرفعال و فاصلهٔ بررسی (دقیقه) از داخل رابط ذخیره و بدون نیاز به متغیر محیطی اعمال می‌شود","کارت «دیپلویر محلی پایتون (deployer4)» وضعیت پورت ۸۰۰۱، مسیر /deploy/ و سلامت آن را نشان می‌دهد و با یک کلیک سلامت‌سنجی می‌شود","گزارش تغییرات حالا در بالا نسخهٔ پایتون، Flask و وضعیت به‌روزرسانی خودکار را هم نمایش می‌دهد"]},
    {"version":"10.189","date":"2026-09-20","title":"بهبود اولویت تشخیص صفحه‌بندی و الگوی مسیر","items":["تشخیص query/path قبل از دکمه بعدی انجام می‌شود تا صفحه‌های دارای شماره صفحه (۱،۲،۳) به‌جای next به‌درستی به‌عنوان query یا /page/ شناخته شوند","الگوی مسیر همیشه به صورت ساده «/page/{page}/» برمی‌گردد تا با page_url که base_path را جدا می‌افزاید، مسیر تکراری /category/category/page تولید نشود","حالت scroll/none هم بدون خطا مدیریت می‌شود"]},
    {"version":"10.188","date":"2026-09-20","title":"تشخیص خودکار مکانیزم صفحه‌بندی","items":["موتور استخراج اکنون نوع صفحه‌بندی (پارامتر Query، مسیر /page/، الگوی کامل URL، دکمه صفحه بعد، اسکرول و بدون صفحه‌بندی) را از روی لینک‌های صفحه به‌صورت خودکار تشخیص می‌دهد","حالت جدید «خودکار» به فهرست صفحه‌بندی اضافه شد؛ اگر انتخاب شود یا مقدار فعلی بی‌اثر باشد، بهترین مکانیزم از روی HTML همان صفحه کشف و برای صفحات بعد استفاده می‌شود","مقدار کشف‌شده (مثلاً نام پارامتر page/p، الگوی /page/{page}/ یا سلکتور دکمه بعدی) در لاگ ثبت و در صورت وجود پروفایل، برای اجراهای بعدی ذخیره می‌شود","اسکرول بینهایت و حالت بدون صفحه‌بندی هم تشخیص داده می‌شود تا استخراج بیهوده صفحه تکراری نزند"]},
    {"version":"10.187","date":"2026-09-20","title":"تثبیت انتخاب الگوی مسیر بدون پرش به گزینه دیگر","items":["گزینه «الگوی مسیر با {page}» (path_pattern) دیگر پس از ذخیره به «مسیر /page/2/» (path_page) تغییر نمی‌کند؛ هر حالت صفحه‌بندی دقیقاً همان که انتخاب شده می‌ماند","نگاشت صفحه‌بندی بین داشبورد Node و بک‌اند برای همهٔ ۸ حالت (query_page/query_custom/path_page/path_pattern/full_pattern/next_selector/none/scroll) یکسان و برگشت‌پذیر شد","ذخیره از هر دو رابط (کلاسیک و Node) مقدار page_value را دست‌نخورده نگه می‌دارد و صفحه‌بندی مسیر حتی با مقدار ساده «page» به «/page/{page}/» نرمال‌سازی می‌شود"]},
    {"version":"10.186","date":"2026-09-20","title":"رفع ناهماهنگی صفحه‌بندی مسیر و الگو","items":["صفحه‌بندی «مسیر» (/page/{page}/) دیگر به «Query» برنمی‌گردد؛ هر چهار حالت مسیر، URL کامل، پارامتر و لینک بعدی دقیقاً همان که انتخاب شده باقی می‌ماند","الگوی مسیر حتی اگر فقط «page» وارد شده باشد خودکار به «/page/{page}/» تبدیل می‌شود و وجود یا عدم وجود {page} و اسلش‌ها نرمال‌سازی می‌شود","الگوی URL کامل بدون {page} هم خودکار ترمیم و اسلش‌های اضافی و حذف /page/۱ تکراری برطرف شد؛ query هم اگر بجای نام پارامتر الگو داده شود به مسیر تبدیل می‌شود","نگاشت صفحه‌بندی بین داشبورد Node و بک‌اند Python یکسان‌سازی شد (query_page/path_page/full_pattern) و مقدار page_value پس از استخراج دست نمی‌خورد"]},
    {"version":"10.185","date":"2026-09-20","title":"کشف خودکار سلکتورهای سالم اما بی‌نتیجه","items":["علاوه بر نحو نامعتبر، سلکتورهایی که از نظر نوشتاری درست‌اند ولی روی صفحهٔ فعلی هیچ نتیجه‌ای نمی‌دهند هم ناقص حساب و خودکار ترمیم می‌شوند","در فهرست، هر فیلد (عنوان، قیمت، لینک، تصویر) که روی نمونهٔ کارت‌ها ۰ تطابق داشته باشد با پیشنهاد تأییدشده جایگزین می‌شود؛ پروفایل دستی فقط در همان فیلد بروز می‌شود","در صفحهٔ جزئیات، هر سلکتور خالی، نامعتبر یا بی‌نتیجه روی همان محصول (با راستی‌آزمایی selector_values) با نمونهٔ پیشنهادی پر و ذخیره می‌شود","ترمیم فقط پس از تأیید روی همان HTML انجام و در لاگ استخراج ثبت می‌شود تا اجرای بعدی بدون تنظیم دستی ادامه یابد"]},
    {"version":"10.184","date":"2026-09-20","title":"کشف خودکار سلکتورهای نامعتبر و ناقص","items":["سلکتور نامعتبر یا ناقص دیگر فقط نادیده گرفته نمی‌شود؛ روی همان صفحهٔ دریافت‌شده به‌صورت خودکار کشف و جایگزین می‌شود","پروفایل‌هایی که هر پنج سلکتور فهرست را دستی پر کرده بودند ولی یکی نامعتبر بود، اکنون فقط همان فیلد خراب ترمیم می‌شود و بقیه دست نمی‌خورد","سلکتورهای خالی یا نامعتبر صفحهٔ جزئیات (گالری، تنوع، وزن و …) هم به همین روش پیدا و در بخش دستی پروفایل ذخیره می‌شوند","نتیجهٔ کشف در لاگ استخراج و عیب‌یابی ثبت و برای اجراهای بعدی پایدار می‌ماند"]},
    {"version":"10.183","date":"2026-09-20","title":"رفع خطای تست سرعت به‌خاطر سلکتور نامعتبر تصویر","items":["سلکتور نامعتبر تصویر (Invalid expression) دیگر کل استخراج و تست سرعت را متوقف نمی‌کند؛ فیلد فقط خالی می‌ماند و محصول از مسیرهای جایگزین خوانده می‌شود","سلکتورهای جزئیات، برچسب، قیمت، گالری و مشخصات هم در برابر عبارت نامعتبر مقاوم شدند؛ ظرف نامعتبر هم به‌جای خطا به تشخیص خودکار می‌افتد","تست سه‌صفحه‌ای دیگر به‌خاطر یک سلکتور خراب، بقیه موتورها را با «سایت در دسترس نیست» رد نمی‌کند"]},
    {"version":"10.182","date":"2026-09-20","title":"تشخیص و رفع خودکار ماندن نسخه روی ۱۰.۱۷۶","items":["نصب‌کننده حالا قبل و بعد از دریافت کد نسخهٔ SRC و RUN را چاپ می‌کند تا علت ماندن روی ۱۰.۱۷۶ فوری دیده شود","اگر SRC پس از fetch هنوز روی نسخه‌های قدیمی ۱۰.۱۷۶/۱۰.۱۸۰/۱۰.۱۸۱ باشد، به‌صورت خودکار یک کلون تازه انجام می‌دهد و پس از کپی، تطابق SRC→RUN را با تلاش دوم و همگام‌سازی تضمین می‌کند","پس از ری‌استارت خودکار، نسخهٔ /health با نسخهٔ فایل مقایسه و در صورت مغایرت، gunicorn قدیمی با sigkill بسته و دوباره بالا می‌آید","پیام‌ها همچنان کاملاً انگلیسی و مراحل 1/6 تا 6/6 باقی ماند"]},
    {"version":"10.181","date":"2026-09-20","title":"تثبیت قطعی به‌روزرسانی ترموکس و رفع ماندن روی ۱۰.۱۷۶","items":["رفع ماندن نسخه روی ۱۰.۱۷۶ پس از نصب: اگر fetch یا reset شکست بخورد حالا به‌جای ماندن روی checkout قدیمی، یک کلون تازه با همان برنچ انجام می‌شود (هم در نصب اصلی curl | bash و هم در scraper4 update)","پس از کپی فایل‌ها، نسخهٔ مبدا و مقصد مقایسه می‌شود و در صورت عدم تطابق یک بار دیگر کپی و همگام‌سازی انجام می‌شود تا RUN/scraper4.py واقعاً نسخهٔ SRC را داشته باشد","پیام‌های مراحل و خطاها همچنان انگلیسی و نصب 1/6 تا 6/6 باقی ماند؛ منطق تازه از ریستارت خودکار قبلی هم پشتیبانی می‌کند تا نسخهٔ جدید بلافاصله سرو شود"]},
    {"version":"10.180","date":"2026-09-20","title":"انگلیسی‌سازی نصب Termux و تثبیت به‌روزرسانی نسخه","items":["همهٔ پیام‌ها و توضیحات داخل tools/install_termux.sh از فارسی به انگلیسی برگردانده شد (درخواست: توضیحات داخل فایل sh انگلیسی باشند)","رفع ایراد «ورژن آپدیت نمی‌شود»: مسیر به‌روزرسانی shallow حالا با fallback کامل امتحان می‌شود و در هر دو بخش do_update و نصب اصلی، fetch و reset با هر دو حالت FETCH_HEAD و origin/branch انجام می‌شود","پس از نصب با curl | bash یا اجرای scraper4 update اگر سرویس در حال اجرا باشد به‌صورت خودکار scraper4 restart انجام می‌شود تا نسخهٔ جدید واقعاً سرو شود؛ قبلاً فایل‌ها به‌روز می‌شدند ولی پروسهٔ قدیمی همچنان نسخهٔ قبلی را نشان می‌داد","پیام‌های مراحل نصب به فرمت انگلیسی 1/6 تا 6/6 و راهنمای نهایی (Run/Stop/Restart/Status/Log/Update, Dashboard, Notes) یکدست شد و گزارش‌های خطا انگلیسی شدند"]},
    {"version":"10.179","date":"2026-09-20","title":"خودنوسازی دستور scraper4 و پر شدن خودکار بخش نسخهٔ کد","items":["همهٔ دستورات start و stop و restart و status و log و update داخل خود فایل install_termux.shتعریف شدند و scraper4 update حالا همان فایل را از مخزن تازه می‌گیرد و دوباره اجرا می‌کند، پس خودِ دستور هم خودکار به‌روز می‌شود","دستور تازهٔ scraper4 restart اضافه شد","تنظیمات منبع به‌روزرسانی از ریپوی قدیمی amphp به fazilatma/new و برنچ arena/01a0bd3f-new متناسب شد؛ مسیر پیش‌فرض هم python-scraper4/scraper4.py است","ریپو و برنچ‌ها در بخش «نسخهٔ کد» خودکار پر می‌شوند: /api/deployer/branches حالا نسخهٔ در حال اجرا، نسخهٔ هر برنچ، وضعیت (جدیدتر/قدیمی‌تر/برابر)، برنچ فعلی و آخرین نسخه را برمی‌گرداند و داشبورد هنگام باز شدن یک اسکن سریع خودکار می‌کند","متن‌های راهنمای بخش نسخه از Cloudflare/Render به به‌روزرسانی خودکار git (هر ۶۰ ثانیه) و دیپلویر چندبرنچ پایتون (deployer4 روی پورت ۸۰۰۱ و مسیر /deploy/) تغییر کرد؛ دکمهٔ «بازگشتن دیپلویر» دیگر به پورت ۸۷۹۰ گره نخورده","کارت «اطلاعات نسخه» حالا مخزن، برنچ، کامیت محلی/دوردست، تعداد کامیت عقب و وضعیت به‌روزرسانی خودکار را نشان می‌دهد","کارت‌های دستورهای نصب و آپدیت با دستورهای واقعی همین نصب پایتون (VPS، Termux، دسکتاپ، ویندوز، دیپلویر) جایگزین نسخه‌های npm شدند","چیدمان بخش نسخهٔ کد مرتب شد: استایل یکدست برای تب‌ها، آکاردئون‌ها، جدول برنچ‌ها (سرستون چسبان و هاور) و کارت‌های دستور نصب"]},
    {"version":"10.178","date":"2026-09-20","title":"کشف خودکار سلکتورهای خالی یا ناقص و ذخیره در سلکتورهای دستی","items":["قبلاً اگر سلکتورهای پروفایل خالی یا ناقص بود، استخراج با خطای «هیچ محصولی استخراج نشد» تمام می‌شد و پرکردن دستی همهٔ فیلدها تنها راه بود","حالا سلکتورها از روی همان صفحهٔ دریافت‌شده خودکار کشف می‌شوند: اول الگوهای آمادهٔ فروشگاه‌ها (ووکامرس و مانند آن) و بعد تحلیل ساختاری کارت‌های محصول","هر پیشنهاد پیش از پذیرش روی صفحهٔ واقعی راستی‌آزمایی می‌شود: ظرف باید تکرارشونده باشد و عنوان داخل همان کارت‌ها پیدا شود","سلکتورهای پیداشده خودکار در سلکتورهای دستی پروفایل ذخیره می‌شوند؛ اجراهای بعدی و تب سلکتورها همان‌ها را می‌بینند","سلکتور دستیِ کامل (هر پنج فیلد فهرست پرشده) هرگز دست نمی‌خورد؛ در مجموعهٔ ناقص هم فقط فیلدهای خالی پر می‌شوند","صفحهٔ جزئیات هم همین رفتار را گرفت: فیلدهای خالی (توضیح، قیمت، گالری، تنوع، برند، اس‌کی‌یو و…) از اولین صفحهٔ محصول واقعی پر و ذخیره می‌شوند","عیب‌یابی استخراج، مراحل «کشف خودکار سلکتورها» و «ذخیرهٔ خودکار سلکتورها» را مثل نسخهٔ Node گزارش می‌کند","دکمهٔ «پیشنهاد خودکار سلکتورها» در داشبورد که تاکنون «در این نسخه در دسترس نیست» می‌گفت، حالا واقعاً کار می‌کند (POST /api/suggest-selectors)"]},
    {"version":"10.177","date":"2026-09-20","title":"نصب‌کنندهٔ اختصاصی ترموکس (اندروید)","items":["نصب با یک دستور روی گوشی: tools/install_termux.sh","دستور scraper4 برای start و stop و status و log و update ساخته می‌شود","هنگام اجرا wake-lock گرفته می‌شود تا اندروید استخراج طولانی را قطع نکند","lxml از بستهٔ آمادهٔ ترموکس نصب می‌شود و کتابخانه‌های اختیاری در صورت شکست، نصب را متوقف نمی‌کنند","رفع اشکال run_scraper4.sh که هنوز از مخزن قدیمی می‌گرفت و فایل‌های داشبورد را کپی نمی‌کرد؛ یعنی /ui روی گوشی ۴۰۴ می‌شد","راهنمای فارسی ترموکس به INSTALL.md اضافه شد"]},
...
def _auto_update_from_data() -> tuple[bool, int]:
    try:
        data = load_data()
        dep = data.get("deploy", {}) if isinstance(data.get("deploy"), dict) else {}
        if "SCRAPER_AUTO_UPDATE" in os.environ:
            enabled = os.environ.get("SCRAPER_AUTO_UPDATE", "1").lower() not in {"0", "false", "off", "no"}
        else:
            enabled = bool(dep.get("auto_update_enabled", True))
        if "SCRAPER_AUTO_UPDATE_INTERVAL" in os.environ:
            interval = max(120, int(os.environ.get("SCRAPER_AUTO_UPDATE_INTERVAL", "300")))
        else:
            try:
                interval = max(120, min(86400, int(dep.get("auto_update_interval", 300))))
            except (TypeError, ValueError):
                interval = 300
...
def default_data() -> dict[str, Any]:
    return {
        ...
        "deploy": {
            "repo": "fazilatma/new", "branch": "arena/01a0bd3f-new",
            "branches": ["arena/01a0bd3f-new"],
            "path": "python-scraper4/scraper4.py",
            "github_token": "", "reload_file": "", "check_on_load": False,
            "auto_update_enabled": True, "auto_update_interval": 300,
        },
        "last_result": [],
        "extract_jobs": {},
        "woo_jobs": {},
...
def selector_is_invalid(selector: str) -> bool:
    """True if selector syntax is invalid (CSS or XPath). Empty is not invalid."""
    sel = clean_text(selector)
    if not sel:
        return False
    if is_xpath(sel):
        try:
            from lxml import html as _lhtml  # type: ignore
            _lhtml.fromstring("<div></div>").xpath(sel)
            return False
        except Exception:
            return True
    try:
...
def select_value(node: Tag, selector: str, kind: str, base: str) -> str:
    ...
    try:
        ...
        # Invalid selector must never abort extraction/benchmark; just skip this field.
        # Diagnostics still report which selector was dead via selector-evidence.
        return ""
...
def parse_selectors(soup: BeautifulSoup, base: str, selectors: dict[str, str]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # Bad container selector: fall back to heuristic detection instead of failing.
        return []
    out: list[dict[str, Any]] = []
    for node in nodes:
...
def parse_detail_fields(soup: BeautifulSoup, base: str, selectors: dict[str, str]) -> dict[str, Any]:
    ...
    for field, (fallback, limit) in mapping.items():
        ...
        try:
            ...
            # Skip invalid detail selector; fallback will still provide data if possible.
            continue
        if node:
    ...
    try:
        tag_nodes=soup.select(tag_selector)
    except Exception:
        tag_nodes=[]
    tags=[]
    ...
    try:
        image_nodes=soup.select(gallery_selector)
    except Exception:
        image_nodes=[]
    images=[]
    ...
    try:
        attribute_rows=soup.select(attribute_selector)
    except Exception:
        attribute_rows=[]
...
def page_url(original: str, page: int, kind: str, value: str) -> str:
    ...
    # Normalize aliases so both Python classic and Node dashboard vocabularies work.
    k = (kind or "query").strip().lower()
    if k in ("path", "path_page", "path_pattern", "pattern_path"):
        k = "path"
    elif k in ("full", "full_pattern", "pattern", "custom_full"):
        k = "full"
    elif k in ("query", "query_page", "query_custom", "param", "custom", "query_string"):
        k = "query"
    elif k in ("next", "next_selector", "link"):
        k = "next"
    if k == "path":
        # Tilde-style for emalls.ir: "~page~{page}" — delimiter is ~ not /
        raw_tilde = (value or "").strip()
        if "{page}" in raw_tilde and "~" in raw_tilde:
            pattern = raw_tilde
            replacement = pattern.replace("{page}", str(page))
            root = f"{parsed.scheme}://{parsed.netloc}"
            # Strip previous tilde suffix like "~page~2"
            import re as _re_tilde
            base_path = _re_tilde.sub(r"~page~\d+/?$", "", parsed.path, flags=_re_tilde.I)
            if "~" in pattern:
                prefix = pattern.split("{page}")[0]
                if prefix and prefix.lower() != "~page~":
                    try:
                        base_path = _re_tilde.sub(_re_tilde.escape(prefix) + r"\d+/?$", "", base_path, flags=_re_tilde.I)
                    except _re_tilde.error:
                        pass
            if replacement.startswith("~"):
                return root + base_path.rstrip("/") + replacement
            elif replacement.startswith("/"):
                return root + base_path.rstrip("/") + replacement
            else:
                return root + base_path.rstrip("/") + (replacement if replacement.startswith("/") else "/" + replacement)
        # Robust path-pattern handling: accept "/page/{page}/", "page", "/p/{page}" etc.
        raw = (value or "").strip()
        if not raw:
            pattern = "/page/{page}/"
        elif "{page}" in raw:
            ...
            # "/page/" or "/p/" -> append placeholder
            pattern = raw.rstrip("/") + "/{page}/"
            if not pattern.startswith("/"):
                pattern = "/" + pattern
        else:
            # Bare word like "page" or "p" -> "/page/{page}/"
            token = raw.strip("/") or "page"
            if token.lower() == "page":
                pattern = "/page/{page}/"
            else:
                pattern = f"/{token}/{{page}}/"
        replacement = pattern.replace("{page}", str(page))
        root = f"{parsed.scheme}://{parsed.netloc}"
        # Strip previous page suffix like "/page/2" or "/page/2/" to avoid duplication.
        base_path = re.sub(r"/page/\d+/?$", "", parsed.path.rstrip("/"), flags=re.I)
        # Also strip generic pattern prefix if user used custom token e.g. "/p/2"
        # Try to strip "/<token>/\d+" for token case
        if pattern.startswith("/") and "{page}" in pattern:
            prefix = pattern.split("{page}")[0].rstrip("/")
            if prefix and prefix != "/page":
                base_path = re.sub(re.escape(prefix) + r"/\d+/?$", "", base_path, flags=re.I)
    ...
    if k == "full":
        ...
        if not raw:
            return original
        if "{page}" not in raw:
            # User gave a full URL without placeholder — append sanely.
            raw = raw.rstrip("/") + "/{page}/"
        return raw.replace("{page}", str(page))
    if k == "next":
        # "next" never uses page_url for navigation; fall back to query style if called.
        k = "query"
        value = "page"
    # Query param style
    param = (value or "page").strip() or "page"
    # If value looks like a path pattern mistakenly put in query mode, handle as path.
    if "{page}" in param or "/" in param or "~" in param:
        return page_url(original, page, "path", param)
...
def detect_pagination(soup: BeautifulSoup, base: str) -> tuple[str, str]:
    """Heuristic auto-detection of pagination mechanism from the first page HTML.

    Returns (kind, value) where kind is one of query/path/full/next/none/scroll
    and value is the corresponding page_value (param name, pattern, or selector).
    Twin of the Node detectPagination, but tuned for Python's BeautifulSoup.
    """
    try:
        # 1) Collect pagination links first (query/path/full) — prioritize over next
        pagination_containers = soup.select(".pagination, .pager, .pages, nav.pagination, ul.pagination, div.pagination, [class*='pagination'], [class*='pager']")
        links: list[Any] = []
        if pagination_containers:
            for cont in pagination_containers:
                links.extend(cont.select("a[href]"))
        else:
            for a in soup.select("a[href]"):
                href = a.get("href") or ""
                txt = clean_text(a.get_text())
                if re.search(r"page|p=\d|/page/\d", href, re.I) or txt.isdigit() or txt.lower() in ("2", "3", "4", "5"):
                    links.append(a)
        query_hits = 0
        path_hits = 0
        full_hits = 0
        ...
        for a in links[:20]:
            href = a.get("href") or ""
            if re.search(r"[?&](page|p|pg|pagenum|page_num)=\d+", href, re.I):
                query_hits += 1
                m = re.search(r"[?&](page|p|pg|pagenum|page_num)=", href, re.I)
                if m:
                    query_param = m.group(1).lower()
            elif re.search(r"/page/\d+", href, re.I):
                path_hits += 1
                if path_hits == 1:
                    # Generic WordPress-style path pattern — keep it simple to avoid duplication
                    # e.g., href="/category/page/2/" -> pattern "/page/{page}/" (page_url will prepend base_path)
                    path_pattern = "/page/{page}/"
            elif re.search(r"~page~\d+", href, re.I):
                path_hits += 1
                if path_hits == 1:
                    # Emalls.ir style: "...~Category~123~page~2" -> pattern "~page~{page}"
                    path_pattern = "~page~{page}"
            elif href.startswith("http"):
                if re.search(r"\d+", href) and ("page" in href.lower() or clean_text(a.get_text()).isdigit()):
                    full_hits += 1
                    if not full_pattern:
                        full_pattern = re.sub(r"\d+", "{page}", href, count=1)
        if query_hits >= 2:
            return ("query", query_param)
        if path_hits >= 2:
            return ("path", path_pattern)
        if full_hits >= 2:
            return ("full", full_pattern)
        # 2) Next button — if no clear query/path, try next
        next_candidates = [
            'a[rel="next"]',
            'a.next',
            '.pagination a.next',
        ...
        for sel in next_candidates:
            try:
                ...
                if node and node.get("href"):
                    href = absolute_url(node.get("href"), base)
        ...
        for a in soup.select("a[href]"):
            ...
            href = a.get("href") or ""
            if txt in ("بعدی", "next", "»", "›", "→", ">") or "بعدی" in txt or "next" in txt:
                if a.get("rel") and "next" in str(a.get("rel")).lower():
                    return ("next", 'a[rel="next"]')
                cls = a.get("class")
                if cls:
                    first_cls = str(cls[0]).strip() if isinstance(cls, list) else str(cls).split()[0]
                    if first_cls:
                        cand = f'a.{first_cls}'
                        try:
                            if len(soup.select(cand)) >= 1:
                                parent = a.parent
                                if parent and parent.get("class"):
                                    pcls = str(parent.get("class")[0]) if isinstance(parent.get("class"), list) else str(parent.get("class")).split()[0]
                                    if pcls and ("pagination" in pcls.lower() or "pager" in pcls.lower()):
                                        cand = f'.{pcls} a.{first_cls}'
                                return ("next", cand)
                        except Exception:
                            pass
                return ("next", 'a[rel="next"]')
        # 3) No pagination links
        if not links:
            return ("none", "")
        if links:
            return ("query", query_param)
...
# ---------------------------------------------------------------------------
# Automatic selector discovery and repair (10.178)
#
# Empty or incomplete manual selectors used to be a dead end: the run died
# with «هیچ محصولی استخراج نشد … آدرس، سلکتور یا اتصال شبکه را بررسی کنید» and
# the only way forward was filling every field by hand. The Node twin solves
# this with ensureListSelectors/discoverListSelectorsFromHtml/suggestSelectors
# in worker-src/scraper.ts; this is the Python port. Same two passes (curated
# shop candidates first, structural card inference second), same verification
# gate (container counted page-wide, title/price/link/image INSIDE the cards),
# fully custom selectors are never touched, and every accepted proposal is
# written back to the profile's manual selectors so the selectors tab and all
# later runs see them.
# ---------------------------------------------------------------------------
LIST_SELECTOR_KEYS = ("container", "title", "price", "link", "image")
# The classic Python UI also keeps a list-level sku field; discovery may fill it.
LIST_DISCOVERY_KEYS = LIST_SELECTOR_KEYS + ("sku",)

# Curated e-commerce candidates — mirror of SUGGESTION_CANDIDATES in
# worker-src/scraper.ts. Platform-specific guesses stay first; the generic
# ones only win when nothing above matched. Structural inference below is the
# real fallback when none of these exist either.
LIST_CANDIDATES: dict[str, dict[str, Any]] = {
    "container": {"selectors": ["li.product", "article.product", ".products .product", ".product-card", ".product-item", "[data-product-id]", "article", "[class*='product-card']", "[class*='product-item']", "[class*='product-box']", "[data-product]", ".grid-item", ".product", ".product-box", ".item-card"]},
    "title": {"selectors": [".woocommerce-loop-product__title", ".product-title", ".card-title", "h2", "h3", "[itemprop='name']", ".product-name", "[class*='product-title']", "[class*='product-name']", ".name", "h4"]},
    "price": {"selectors": [".price ins", ".sale-price", ".price", "[itemprop='price']", ".amount", "[class*='price']", ".money", "[data-price]", ".product-price"]},
    ...
    "sku": {"selectors": [".sku", "[itemprop='sku']", "[data-sku]", "[data-product-sku]", "[class*='sku']"]},
}

# Detail-page candidates keyed by the Python detail_selectors names that
# parse_detail_fields reads (the Node ids shortDesc/longDesc/detailImage map
# to short_desc/long_desc/image — ui_bridge.node_to_profile does the rename).
DETAIL_CANDIDATES: dict[str, dict[str, Any]] = {
    "short_desc": {"selectors": [".woocommerce-product-details__short-description", ".short-description", "[itemprop='description']", ".product-info", ".short-desc", "[class*='short-description']"]},
    "long_desc": {"selectors": ["#tab-description", ".woocommerce-Tabs-panel--description", ".product-description", ".description", ".product-tabs", "[class*='description']"]},
    ...
    "price": {"selectors": ["[itemprop='price']", ".price ins", ".sale-price", ".price", ".amount", "[class*='price']"]},
    "image": {"type": "image", "selectors": [".woocommerce-product-gallery__image img", ".product-main-image img", "img.wp-post-image", "[itemprop='image']"]},
...
# Python detail key -> Node dashboard field id (ui/dashboard.js writes the
# suggestions into $('sel-'+id) using exactly these ids).
DETAIL_PY_TO_NODE = {"short_desc": "shortDesc", "long_desc": "longDesc", "image": "detailImage"}

_BLOCK_TAGS = ["div", "ul", "ol", "li", "table", "section", "article", "header",
               "footer", "main", "form", "p", "h1", "h2", "h3", "h4", "h5", "h6"]
...
def list_selectors_status(selectors: Optional[dict[str, Any]]) -> str:
    """empty | partial | custom — are the five list fields filled by hand?"""
    values = [clean_text((selectors or {}).get(key)) for key in LIST_SELECTOR_KEYS]
    if not any(values):
        return "empty"
    if not all(values):
        return "partial"
...
def page_soup(page: Any) -> BeautifulSoup:
    """Accept already-parsed soup or raw HTML without parsing twice."""
...
def selector_values(root: Any, selector: str, kind: str, base: str) -> list[str]:
    """Every non-empty value a CSS selector produces under root ([] if invalid)."""
    sel = clean_text(selector)
    if not sel or is_xpath(sel):
        return []
    try:
        matches = list(root.select(sel))
    except Exception:
        return []
    out: list[str] = []
    for match in matches:
        if kind == "link":
            value = absolute_url(match.get("href") or (match.find("a", href=True) or {}).get("href", ""), base)
        elif kind == "image":
            value = ""
            for attr in ("data-zoom", "data-large", "data-src", "data-lazy-src", "src"):
                if match.get(attr):
                    ...
                    break
            if not value:
                img = match if match.name == "img" else match.find("img")
                value = image_value(dict(img.attrs) if img else {}, base)
        else:
            value = clean_text(match.get_text(" ", strip=True))
        if value:
...
def verify_list_selectors(page: Any, base: str, selectors: Optional[dict[str, str]]) -> dict[str, Any]:
    """Check list selectors the way extraction reads them.

    Container matches are counted page-wide, but title/price/link/image must
    resolve INSIDE the cards — otherwise the evidence is the classic
    contradiction: green page-wide, zero products. Twin of the Node
    verifyListSelectors (descendant-selector evidence), scoped per card.
    """
    selectors = selectors or {}
    empty: dict[str, Any] = {"containerCount": 0, "cardsSampled": 0,
                             "title": {"count": 0, "sample": ""}, "price": {"count": 0, "sample": ""},
                             "link": {"count": 0, "sample": ""}, "image": {"count": 0, "sample": ""},
                             "ok": False}
    container = clean_text(selectors.get("container"))
    if not container:
        return empty
    soup = page_soup(page)
    try:
        cards = list(soup.select(container))
    except Exception as exc:
        return {**empty, "error": f"سلکتور ظرف نامعتبر است: {exc}"}
    container_count = len(cards)
    if container_count < 1:
        return empty
    sample = cards[:12]

    def evidence_for(key: str, kind: str, transform: Optional[Callable[[str], str]] = None) -> dict[str, Any]:
        sel = clean_text(selectors.get(key))
        hits: list[str] = []
        if sel:
            for card in sample:
                try:
                    value = select_value(card, sel, kind, base)
                except Exception:
                    value = ""
                if transform:
                ...
                if value:
                    hits.append(value)
        return {"count": len(hits), "sample": clean_text(hits[0])[:200] if hits else ""}

    title_ev = evidence_for("title", "title")
    price_ev = evidence_for("price", "price", extract_price)
    link_ev = evidence_for("link", "link")
    image_ev = evidence_for("image", "image")
    ...
    needed = max(1, (min(container_count, 12) + 1) // 2)
    return {"containerCount": container_count, "cardsSampled": len(sample),
            "title": title_ev, "price": price_ev, "link": link_ev, "image": image_ev,
            "ok": container_count >= 2 and title_ev["count"] >= needed}


def _css_escape_ident(value: str) -> str:
    out = re.sub(r"[^a-zA-Z0-9_-]", lambda m: "\\" + m.group(0), value)
    return ("\\3" + out[0] + " " + out[1:]) if out[:1].isdigit() else out


def _stable_classes(node: Tag) -> list[str]:
    raw = node.get("class") or []
    names = raw if isinstance(raw, list) else re.split(r"\s+", str(raw))
    stable = [n for n in names if n and len(n) <= 40 and not _VOLATILE_CLASS_RE.match(n) and not _HASH_CLASS_RE.match(n)]
    rank = lambda n: (100 if re.search(r"[^a-zA-Z0-9_-]", n) else 0) + len(n)
    return sorted(set(stable), key=rank)


def _selector_for_node(node: Tag) -> str:
    """Minimal `tag.class` selector for a card/leaf (relative-safe inside a card)."""
    tag = (node.name or "div").lower()
    if not re.fullmatch(r"[a-z][a-z0-9]*", tag):
        tag = "div"
    classes = _stable_classes(node)
    if len(classes) >= 2:
        return f"{tag}.{_css_escape_ident(classes[0])}.{_css_escape_ident(classes[1])}"
    if classes:
        return f"{tag}.{_css_escape_ident(classes[0])}"
    return tag


def _derive_structural_field_selectors(cards: list[Tag]) -> Optional[dict[str, Any]]:
    """Derive title/price selectors from inside sampled cards of one cluster."""
    title_votes: dict[str, dict[str, int]] = {}
    price_votes: dict[str, dict[str, int]] = {}
    card_is_link = 0
    for card in cards:
        if card.name == "a" and card.get("href"):
            card_is_link += 1
        # Title: headings/itemprop first, else the longest mid-length text leaf.
        title_sig = ""
        heading = card.find(["h1", "h2", "h3", "h4"]) or card.find(attrs={"itemprop": "name"})
        if heading is not None and heading is not card:
            text = clean_text(heading.get_text(" ", strip=True))
            if 8 <= len(text) <= 200 and not extract_price(text):
                title_sig = _selector_for_node(heading)
        if not title_sig:
            best_len = 0
            for leaf in card.find_all(["span", "div", "p", "a", "li", "td", "strong", "b"]):
                if leaf is card or leaf.find(_BLOCK_TAGS):
                    continue
                text = clean_text(leaf.get_text(" ", strip=True))
                if 15 <= len(text) <= 160 and not extract_price(text) and len(text) > best_len:
                    best_len, title_sig = len(text), _selector_for_node(leaf)
        if title_sig:
            vote = title_votes.setdefault(title_sig, {"count": 0, "bonus": 2 if re.match(r"^h[1-4]\.", title_sig) else 0})
            vote["count"] += 1
        # Price: the SHORTEST price-shaped leaf wins — wrappers that also hold
        # the title lose to the element that carries just the price.
        best_price: Optional[dict[str, Any]] = None
        for leaf in card.find_all(True):
            if leaf is card or leaf.find(_BLOCK_TAGS):
                continue
            text = clean_text(leaf.get_text(" ", strip=True))
            if not text or len(text) > 80 or not extract_price(text):
                continue
            if best_price is None or len(text) < best_price["length"]:
                best_price = {"sig": _selector_for_node(leaf), "length": len(text)}
        if best_price:
            vote = price_votes.setdefault(best_price["sig"], {"count": 0, "length": best_price["length"]})
            vote["count"] += 1
    if not title_votes:
        return None
    title_winner = max(title_votes.items(), key=lambda kv: kv[1]["count"] * 10 + kv[1]["bonus"])[0]
    price_winner = sorted(price_votes.items(), key=lambda kv: (-kv[1]["count"], kv[1]["length"]))[0][0] if price_votes else ""
    return {"title": title_winner, "price": price_winner, "cardIsLink": card_is_link * 2 >= len(cards)}


def _infer_structural_list_selectors(soup: BeautifulSoup, base: str) -> Optional[dict[str, Any]]:
    """Structural card inference: cluster link+image cards by their tag+class
    signature and treat the largest repeating cluster as the product grid.
    Produces reusable CSS selectors, so every later page and run works with
    them. Twin of inferStructuralListSelectors in worker-src/scraper.ts."""
    groups: dict[str, dict[str, Any]] = {}
    anchors = soup.select("a[href]")[:800]
    if len(anchors) < 2:
        return None
    for anchor in anchors:
        ...
        if not href or href == "#" or href.lower().startswith("javascript:"):
            continue
        # The anchor itself plus up to four enclosing card-like elements.
        chain: list[Tag] = [anchor]
        node: Any = anchor
        ...
        for card in chain:
            ...
            if len(text) < 12 or len(text) > 1500:
                continue
            if not card.find("img"):
                continue
            if len(card.select("a[href]")) > 4:
                continue
            signature = _selector_for_node(card)
            if "." not in signature and card.name not in ("li", "article"):
                continue
            group = groups.setdefault(signature, {"cards": [], "seen": set(), "priceHits": 0})
            if id(card) in group["seen"]:
                continue
            group["seen"].add(id(card))
            group["cards"].append(card)
            if extract_price(text):
                group["priceHits"] += 1
    clusters = sorted(
        ({"selector": sig, "cards": g["cards"], "priceHits": g["priceHits"]}
         for sig, g in groups.items() if len(g["cards"]) >= 2),
        key=lambda c: -(len(c["cards"]) * (1 + c["priceHits"])),
    )
    for cluster in clusters[:5]:
        derived = _derive_structural_field_selectors(cluster["cards"][:8])
        if not derived or not derived.get("title"):
            continue
        link_selector = cluster["selector"] if derived.get("cardIsLink") else "a[href]"
        proposed = {"container": cluster["selector"], "title": derived["title"],
                    "link": link_selector, "image": "img"}
        if derived.get("price"):
            proposed["price"] = derived["price"]
        verified = verify_list_selectors(soup, base, proposed)
        if not verified["ok"]:
            continue
        return {"selectors": proposed,
                "evidence": {"container": {"count": verified["containerCount"], "sample": cluster["selector"]},
                             "title": verified["title"], "price": verified["price"],
                             "link": verified["link"], "image": verified["image"]}}
    return None


def discover_list_selectors(page: Any, base: str) -> dict[str, Any]:
    """Find list selectors for a page whose profile never configured them.

    Pass 1 tests the curated shop candidates (fast, precise on known
    platforms). Pass 2 — structural inference — handles everything else.
    Every proposal is verified against the same HTML before it is returned,
    best-of ranking first (merged, then structural alone, then curated alone),
    so a stale curated stowaway can never veto a working structural set.
    """
    soup = page_soup(page)
    selectors: dict[str, str] = {}
    evidence: dict[str, Any] = {}
    for field in LIST_DISCOVERY_KEYS:
        config = LIST_CANDIDATES[field]
        kind = str(config.get("type", "text"))
        minimum = 2 if field == "container" else 1
        for candidate in config["selectors"]:
            values = selector_values(soup, candidate, kind, base)
            if len(values) >= minimum:
                selectors[field] = candidate
                evidence[field] = {"count": len(values), "sample": clean_text(values[0])[:200], "via": "curated"}
                break
    curated_selectors = dict(selectors)
    curated_evidence = dict(evidence)
    structural: Optional[dict[str, Any]] = None
    if not selectors.get("container") or not selectors.get("title"):
        try:
            structural = _infer_structural_list_selectors(soup, base)
        except Exception:
            structural = None
        if structural:
            for key, value in structural["selectors"].items():
                if value and not selectors.get(key):
                    selectors[key] = value
                    evidence[key] = {**(structural["evidence"].get(key) or {}), "via": "structural"}
    method = ("curated" if not structural else
              ("mixed" if curated_selectors.get("container") and curated_selectors.get("title") else "structural"))
    candidates: list[tuple[dict[str, str], dict[str, Any], str]] = [(selectors, evidence, method)]
    if structural:
        candidates.append((structural["selectors"], structural["evidence"], "structural"))
    candidates.append((curated_selectors, curated_evidence, "curated"))
    container_count = 0
    for sel, ev, meth in candidates:
        if not sel.get("container") or not sel.get("title"):
            continue
        verified = verify_list_selectors(soup, base, sel)
        container_count = verified["containerCount"]
        if verified["ok"]:
            return {"selectors": sel, "evidence": ev, "method": meth, "containerCount": container_count}
    return {"selectors": {}, "evidence": {}, "method": "none", "containerCount": container_count}


def ensure_list_selectors(page: Any, base: str, selectors: Optional[dict[str, str]]) -> dict[str, Any]:
    """Repair empty/incomplete list selectors from already-fetched page HTML.

    Custom (all five list fields hand-filled) selectors pass through
    untouched. Otherwise verify the current set; a set that verifies only gets
    its EMPTY fields topped up, a broken/empty set is replaced by the verified
    discovery result. Nothing is adopted unless the merged set verifies on the
    same page. Twin of ensureListSelectors in worker-src/scraper.ts.
    """
    current = {k: clean_text(v) for k, v in dict(selectors or {}).items()}
    status = list_selectors_status(current)
    verification = verify_list_selectors(page, base, current)
    working_now = verification["ok"]
    has_invalid = any(selector_is_invalid(current.get(k, "")) for k in LIST_SELECTOR_KEYS if current.get(k))
    # Semantically broken = syntactically invalid OR valid but yields 0 hits on the live sample.
    has_broken = has_invalid or any(
        current.get(k) and verification.get(k, {}).get("count", 0) == 0
        for k in ("title", "price", "link", "image", "sku") if k in verification
    ) or (current.get("container") and verification.get("containerCount", 0) == 0)
    if status == "custom" and not has_broken and working_now:
        return {"selectors": current, "method": "", "status": status}
    found = discover_list_selectors(page, base)
    if found["method"] == "none" or not found["selectors"].get("container") or not found["selectors"].get("title"):
        return {"selectors": current, "method": "", "status": status}
    if working_now:
        # Fill empty, syntactically invalid, or semantically empty (0 hits) fields; keep valid custom untouched.
        def _needs_fix(key: str) -> bool:
            if not current.get(key):
                return True
            if selector_is_invalid(current.get(key, "")):
                return True
            if key == "container" and verification.get("containerCount", 0) == 0:
                return True
            ev = verification.get(key)
            if isinstance(ev, dict) and ev.get("count", 0) == 0:
                return True
            return False
        discovered = {k: v for k, v in found["selectors"].items() if v and _needs_fix(k)}
    else:
        discovered = {k: v for k, v in found["selectors"].items() if v}
    if not discovered:
        return {"selectors": current, "method": "", "status": status}
    merged = {**current, **discovered}
    if verify_list_selectors(page, base, merged)["ok"]:
        return {"selectors": merged, "discovered": discovered, "method": found["method"],
                "evidence": found["evidence"], "containerCount": found["containerCount"], "status": status}
    return {"selectors": current, "method": "", "status": status}


def suggest_detail_selectors(page: Any, base: str) -> dict[str, Any]:
    """Curated detail-field proposals for one product page (Python keys)."""
    soup = page_soup(page)
    selectors: dict[str, str] = {}
    evidence: dict[str, Any] = {}
    for field, config in DETAIL_CANDIDATES.items():
        kind = str(config.get("type", "text"))
        for candidate in config["selectors"]:
            values = selector_values(soup, candidate, kind, base)
            if values:
                selectors[field] = candidate
                evidence[field] = {"count": len(values), "sample": clean_text(values[0])[:200]}
                break
    return {"selectors": selectors, "evidence": evidence}


def ensure_detail_selectors(page: Any, base: str, selectors: Optional[dict[str, str]]) -> dict[str, Any]:
    """Fill EMPTY detail-selector fields only; hand-set fields are never touched."""
    current = {k: clean_text(v) for k, v in dict(selectors or {}).items()}
    soup = page_soup(page)
    missing: list[str] = []
    for key in DETAIL_CANDIDATES:
        sel = clean_text(current.get(key))
        if not sel or selector_is_invalid(sel):
            missing.append(key)
            continue
        kind = str(DETAIL_CANDIDATES[key].get("type", "text"))
        try:
            vals = selector_values(soup, sel, kind, base)
        except Exception:
            vals = []
        if not vals:
            # Also try raw node existence for cases where kind mismatch hides nodes
            try:
                nodes = query_nodes(soup, sel)
                if not nodes:
                    missing.append(key)
                else:
                    # nodes exist but yielded no typed value — still considered incomplete
                    missing.append(key)
            except Exception:
                missing.append(key)
    if not missing:
        return {"selectors": current, "discovered": {}, "evidence": {}}
    suggested = suggest_detail_selectors(page, base)
    discovered = {k: v for k, v in suggested["selectors"].items() if k in missing and v}
    return {"selectors": {**current, **discovered}, "discovered": discovered,
...
def auto_selectors(url: str, mode: str = "all") -> dict[str, Any]:
    ...
    ui/dashboard.js writes the result straight into $('sel-'+id) inputs.
    """
    source = public_http_url(str(url or ""))
    mode = clean_text(mode).lower()
    if mode not in ("list", "detail", "all"):
        mode = "all"
    cfg = load_data()
    ...
    evidence: dict[str, Any] = {}
    if mode in ("list", "all"):
        found = discover_list_selectors(soup, result.url)
        selectors.update({k: v for k, v in found["selectors"].items() if v})
        evidence.update(found["evidence"])
        evidence["discoveryMethod"] = found["method"]
        evidence["containerCount"] = found["containerCount"]
    if mode in ("detail", "all"):
        suggested = suggest_detail_selectors(soup, result.url)
        for key, value in suggested["selectors"].items():
            node_key = DETAIL_PY_TO_NODE.get(key, key)
            selectors[node_key] = value
            evidence[node_key] = suggested["evidence"][key]
...
def persist_profile_auto_selectors(profile_name: str,
    ...
    """Save auto-discovered selectors into the profile's MANUAL selector fields.

    List selectors are written over the stored ones (they just verified on the
    live page); detail selectors only fill fields the user left empty. Returns
    {"list": …, "detail": …} of what was actually saved ({} on any failure).
    """
    name = clean_text(profile_name)
    if not name:
        return {}
    list_sel = {k: clean_text(v) for k, v in dict(discovered_list or {}).items()
                if clean_text(v) and k in LIST_DISCOVERY_KEYS}
    detail_sel = {k: clean_text(v) for k, v in dict(discovered_detail or {}).items()
                  if clean_text(v) and k in DETAIL_CANDIDATES}
...
def scrape(config: dict[str, Any]) -> ScrapeReport:
    ...
    for number in range(start_page, pages + 1):
        ...
        if _auto_pagination and number == 2 and page_html:
            try:
                _soup_for_detect = BeautifulSoup(page_html, "lxml")
                _detected_kind, _detected_value = detect_pagination(_soup_for_detect, source)
                if _detected_kind and _detected_kind != "none":
                    pag_kind, pag_value = _detected_kind, _detected_value
                    config["pagination"], config["page_value"] = pag_kind, pag_value
                    report.logs.append(f"صفحه‌بندی خودکار تشخیص داده شد: {pag_kind} → {pag_value}")
                    if task_id:
                        live_task_update(task_id, max(4, round((number-1)/pages*88)+1), "تشخیص صفحه‌بندی", "running", f"{pag_kind}: {pag_value}", done=number-1, total=pages, extracted=len(report.products))
                    # Persist to profile for next runs
                    try:
        ...
        if _effective_kind == "next" and number > 1:
            ...
            url = page_url(source, number, _effective_kind, pag_value)
        # 10.194: detailed pagination URL logging for debug (emalls.ir ~page~)
        try:
            report.logs.append(f"صفحه {number}: URL صفحه‌بندی = {url} (pagination={_effective_kind}:{pag_value})")
            if task_id:
                live_task_update(task_id, max(4, round((number-1)/pages*88)+1), f"صفحه {number} URL", "running", f"{url} · pagination={_effective_kind}:{pag_value}", done=number-1, total=pages, extracted=len(report.products))
        except Exception:
            pass
        rows: list[dict[str, Any]] = []
        ...
        diag: dict[str, Any] = {}
        fetch_error = ""
        # Raw HTML of the page that produced `soup`. Kept so that the
        # selector auto-discovery pass (10.178) can run on the exact HTML
        # that extraction saw, without fetching it a second time.
        page_html = ""
        ...
        # 10.194: if pagination produced duplicate URL, log it (helps emalls debug)
        if url == source and number > 1:
            report.logs.append(f"هشدار صفحه‌بندی: صفحه {number} URL تکراری {url} — الگوی pagination={_effective_kind}:{pag_value} شاید نادرست است")
                ...
                for engine_index,engine in enumerate(engines):
                    ...
                    try:
                        ...
                        if candidate_rows:
                            ...
                            return
                        soup,diag=candidate_soup,{**candidate_diag,"engine":engine}
                    except (FetchError,ValueError) as exc:
                        fetch_error=str(exc)
            _dom_with_backup(fetcher)

        # 10.178/10.184: empty, incomplete OR syntactically invalid list selectors
        # are repaired from the HTML that was just fetched. The proposal is
        # verified on that same page, adopted and written back to the profile's
        ...
        if number == start_page and page_html:
            ...
            if discovered:
                ...
                method=clean_text(ensured.get("method")) or "auto"
                saved=persist_profile_auto_selectors(profile_name,discovered)
                report.modes.add("selector-discovery-"+method)
                report.diagnostics["selector_discovery"]={"method":method,"selectors":discovered,"saved":bool(saved),"container_count":int(ensured.get("containerCount") or 0)}
                pairs="، ".join(f"{k}={v}" for k,v in discovered.items())
                report.logs.append(f"صفحه {number}: سلکتورهای خالی/ناقص خودکار پیدا شد ({method}) و در پروفایل ذخیره شد: {pairs}")
                if task_id:live_task_update(task_id,max(4,round((number-1)/pages*88)+3),"کشف خودکار سلکتورها","running",f"{len(discovered)} سلکتور پیدا و در پروفایل ذخیره شد",done=number-1,total=pages,extracted=len(report.products),stage="selector-discovery")
                try:
                    retry_rows,retry_soup,retry_diag=parse_html(page_html,url,selectors,parse_strategy)
                except Exception as exc:
                    retry_rows,retry_soup,retry_diag=[],None,{};report_error("selector_discovery_reparse",exc)
                if retry_soup is not None:
                    soup=retry_soup;diag={**diag,**retry_diag,"selector_discovery":method}
    ...
    if enrich and report.products:
        ...
        # 10.178: empty detail-selector fields are filled from the first real
        # detail page (curated candidates only, hand-set fields untouched) and
        # saved to the profile's manual selectors, exactly like the list side.
        detail_autofill_done=False
        def _autofill_detail_selectors(dsoup:Any,durl:str)->None:
            nonlocal detail_selectors,detail_autofill_done
            if detail_autofill_done or dsoup is None:
                return
            detail_autofill_done=True
            try:
                ensured_detail=ensure_detail_selectors(dsoup,durl,detail_selectors)
            except Exception as exc:
                report_error("detail_selector_discovery",exc);return
            discovered_detail=ensured_detail.get("discovered") or {}
            if not discovered_detail:
                return
            detail_selectors=dict(ensured_detail.get("selectors") or detail_selectors)
            config["detail_selectors"]=detail_selectors
            saved=persist_profile_auto_selectors(profile_name,None,discovered_detail)
            report.diagnostics["detail_selectors_discovered"]=discovered_detail
            report.logs.append("سلکتورهای خالی صفحهٔ جزئیات خودکار پیدا و در پروفایل ذخیره شد: "+"، ".join(f"{k}={v}" for k,v in discovered_detail.items())+("" if saved else " (پروفایلی برای ذخیره نبود)"))
...
# This fork is the update source: fazilatma/amphp has no ui_bridge.py or ui/,
# so any auto-update pointed at it silently strips the dashboard.
DEPLOY_DEFAULT_REPO = "fazilatma/new"
DEPLOY_DEFAULT_BRANCH = "arena/01a0bd3f-new"
DEPLOY_DEFAULT_PATH = "python-scraper4/scraper4.py"
...
def normalize_branches(raw: Any, fallback: str = "") -> list[str]:
    ...
    if not items:
        items.append(DEPLOY_DEFAULT_BRANCH)
...
def deploy_config(data: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    live = DEPLOY_DEFAULT_BRANCH
    if live not in branches:
        branches = [live] + branches
    if not branches:
        branches = [live]
    return {
        "repo": clean_text(raw.get("repo")) or DEPLOY_DEFAULT_REPO,
        "branch": branches[0],
        "branches": branches,
        "path": clean_text(raw.get("path")) or DEPLOY_DEFAULT_PATH,
...
def _scraper_git_dir() -> str:
    for cand in (os.environ.get("SCRAPER_GIT_DIR", ""), "/opt/amphp", BASE_DIR):
        if cand and os.path.isdir(os.path.join(cand, ".git")):
            return cand
    # Keep the historical cache dir when usable; on Termux / read-only /opt
    # fall back to a writable cache next to the app (then to tempdir).
    legacy = "/opt/amphp"
    try:
...
def schedule_auto_update():
    ...
    if not enabled or request.path.startswith("/api/deploy/"):
        return None
    now = time.time()
    if now - float(AUTO_UPDATE_STATE["last"]) < interval:
        return None
...
def auto_update_loop() -> None:
    ...
    while True:
        try:
            enabled, interval = _auto_update_from_data()
            if enabled and AUTO_UPDATE_LOCK.acquire(blocking=False):
                AUTO_UPDATE_STATE["running"] = True
                try:
                    auto_update_worker()
                except Exception as exc:
...
def start_vps_supervisors() -> None:
    ...
    enabled, _ = _auto_update_from_data()
    if enabled:
...
def get_config():
    ...
    try:
        ...
        deploy.setdefault("branches", [deploy.get("branch", DEPLOY_DEFAULT_BRANCH)])
...
def api_appearance():
    """Font scale persistence without deploy password — survives port changes / localStorage clears."""
    if request.method == "GET":
        data = load_data()
        scale = (data.get("ui_settings") or {}).get("appearance", {}).get("fontScale", 1)
        try:
            scale = float(scale)
        except:
            scale = 1
        return jsonify(ok=True, fontScale=max(0.9, min(1.3, scale)))
    body = request.get_json(silent=True) or {}
    raw = body.get("fontScale", body.get("font_scale", (body.get("appearance") or {}).get("fontScale") if isinstance(body.get("appearance"), dict) else None))
    if raw is None:
        raw = body.get("scale")
    try:
        scale = max(0.9, min(1.3, float(raw)))
    except:
        return jsonify(ok=False, error="fontScale must be 0.9-1.3"), 400
    data = load_data()
    ui = data.setdefault("ui_settings", {})
    app_cfg = ui.setdefault("appearance", {})
    app_cfg["fontScale"] = scale
    save_data(data)
...
def settings():
    if request.method == "GET":
        data = load_data()
        return jsonify(ok=True, network=data.get("network", {}), ui_settings=data.get("ui_settings", {}), deploy=data.get("deploy", {}))
    if not deploy_authorized():return deploy_auth_error()
    body = request.get_json(silent=True) or {}
    data = load_data()
    ...
    if isinstance(body.get("deploy"), dict):
        ...
        if "auto_update_enabled" in body["deploy"]:
            data["deploy"]["auto_update_enabled"] = bool(body["deploy"]["auto_update_enabled"])
        if "auto_update_interval" in body["deploy"]:
            try:
                data["deploy"]["auto_update_interval"] = max(120, min(86400, int(body["deploy"]["auto_update_interval"])))
            except (TypeError, ValueError):
                pass
...
def api_auto_update_get():
    if not deploy_authorized():
        return deploy_auth_error()
    enabled, interval = _auto_update_from_data()
    return jsonify(ok=True, enabled=enabled, interval=interval, state=AUTO_UPDATE_STATE, env_overridden=("SCRAPER_AUTO_UPDATE" in os.environ or "SCRAPER_AUTO_UPDATE_INTERVAL" in os.environ))


@app.post("/api/auto-update")
def api_auto_update_set():
    if not deploy_authorized():
        return deploy_auth_error()
    body = request.get_json(silent=True) or {}
    ...
    data = load_data()
    dep = data.setdefault("deploy", {})
    if enabled is not None:
        dep["auto_update_enabled"] = bool(enabled)
    if interval is not None:
        try:
            dep["auto_update_interval"] = max(120, min(86400, int(interval)))
        except (TypeError, ValueError):
            return jsonify(ok=False, error="فاصله نامعتبر است"), 400
    save_data(data)
    global AUTO_UPDATE_ENABLED, AUTO_UPDATE_INTERVAL
    AUTO_UPDATE_ENABLED, AUTO_UPDATE_INTERVAL = _auto_update_from_data()
    return jsonify(ok=True, enabled=AUTO_UPDATE_ENABLED, interval=AUTO_UPDATE_INTERVAL)


@app.get("/api/deployer/local-status")
def api_deployer_local_status():
    if not deploy_authorized():
        return deploy_auth_error()
    import socket
    status = {"running": False, "port": 8001, "path": "/deploy/", "error": ""}
    try:
        sock = socket.create_connection(("127.0.0.1", 8001), timeout=2)
        sock.close()
        status["running"] = True
    except Exception as exc:
        status["error"] = str(exc)[:200]
    try:
        r = outbound_request("GET", "http://127.0.0.1:8001/deploy/", timeout=5)
        status["http_status"] = r.status_code
        status["running"] = r.status_code < 500
    except Exception:
        pass
...
def profile_save():
    if request.method == "GET":
        data = load_data()
        return jsonify(ok=True, profiles=data.get("profiles", {}), active_profile=data.get("active_profile", ""))
    body = request.get_json(silent=True) or {}
...
def suggest_selectors_alias():
    ...
    mode = clean_text(body.get("mode") or request.args.get("mode") or "all")
    try:
        result = auto_selectors(url, mode)
    except Exception as exc:
        return jsonify(ok=False, error=str(exc)), 400
...
def api_basalam_test():
    ...
    try:
        ...
        if body.get("all") or body.get("test_all") or body.get("all_shops"):
            ...
            for sh in shops:
                scfg = dict(cfg_all)
                scfg["vendor_id"] = int(sh.get("vendor_id") or 0)
                scfg["token"] = str(sh.get("token") or "")
                try:
            ...
            # 10.216 auto-fix: persist corrected names + clean duplicates/zero ids
            auto_fixed = False
            if ok_cnt > 0:
                try:
                    data = load_data()
                            ...
                            for v in bsl.get("vendors") or []:
                                if isinstance(v, dict) and int(v.get("vendor_id") or 0) == vid:
                                    ...
                                    if not clean_text(v.get("name") or "") and real_user:
                                        ...
                                        need_save = True
                                    try:
                                        v["price_val"] = float(v.get("price_val") or 0)
                                    except:
                                        pass
                                    break
                    if need_save:
...
def api_basalam_products():
    ...
    try:
        ...
        # SDK first, REST fallback already via basalam_strategy; keep vendor in error for 500 debugging
...
.store-manager-launch{background:linear-gradient(135deg,#0e7490,#2563eb)!important;border-color:#67e8f977!important;color:#fff!important;box-shadow:0 9px 24px #0369a144!important}.store-manager-modal-card{position:relative;width:min(1680px,100%)!important;padding:0!important;border-color:#38bdf866!important}.store-manager-frame-head{min-height:62px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 15px;background:linear-gradient(135deg,#172554,#0f3d55);border-bottom:1px solid #38506c}.store-manager-frame-title{display:flex;align-items:center;gap:10px;min-width:0}.store-manager-frame-title>i{width:40px;height:40px;display:grid;place-items:center;border-radius:13px;background:#0891b233;color:#67e8f9;font-style:normal;font-size:21px}.store-manager-frame-title b,.store-manager-frame-title small{display:block}.store-manager-frame-title b{color:#fff;font-size:15px}.store-manager-frame-title small{color:#9db0ca;font-size:9px}.store-manager-frame-actions{display:flex;gap:7px}.store-manager-frame-loading{position:absolute;z-index:2;inset:62px 0 0;display:grid;place-items:center;background:#0b1528;color:#67e8f9}.store-manager-frame-loading[hidden]{display:none}.store-manager-modal-card iframe{width:100%;min-height:0;flex:1;border:0;background:#f5f6f8}.store-manager-modal-card iframe[hidden]{display:none}@media(max-width:640px){.store-manager-modal-card{border:0!important;border-radius:0!important}.store-manager-frame-head{min-height:56px;padding:8px}.store-manager-frame-title small,.store-manager-refresh-label{display:none}.store-manager-frame-actions button{padding-inline:9px}.store-manager-frame-loading{inset:56px 0 0}}
...
<section id="deploy" class="pane"><div class="card"><h3>🐍 مدیریت نسخهٔ کد پایتون</h3><div class="note">این نصب پایتون است — نه Worker. همه برنچ‌های کاندید با <code>git fetch</code> بررسی و برنچی که <b>جدیدترین <code>APP_VERSION</code></b> را داشته باشد با <code>git</code> نصب می‌شود. نسخه تازه با <code>python -m py_compile</code> اعتبارسنجی و نسخهٔ فعلی در <code>scraper4.py.bak</code> نگه داشته می‌شود. برای repo خصوصی <code>GITHUB_TOKEN</code> را در همین صفحه یا متغیر محیطی بگذارید. سرویس روی <code>systemd + gunicorn --timeout 0</code> می‌چرخد.</div><div id="deployLocal" class="deploy-local">—</div><button class="green" onclick="deployCheckInstall(true)" id="deployMainBtn" style="width:100%;padding:12px;font-size:14px;margin-top:10px">🔍 بررسی و نصب نسخهٔ جدید (Python)</button><button class="gray" onclick="deployRun()" id="deployUpdateBtn" style="width:100%;padding:11px;margin-top:8px;display:none">⬇ نصب جدیدترین نسخه</button><div id="deployStatus" class="status" style="margin-top:8px">ابتدا تنظیمات را ذخیره و سپس نسخه را بررسی کنید.</div><label class="checkline"><input type="checkbox" id="dep_autocheck" onchange="saveDeploy(true)"> بررسی خودکار هنگام باز/رفرش شدن صفحه (فقط اطلاع می‌دهد؛ نصب با تأیید شماست)</label>
<div class="card" style="margin-top:12px;border-color:#22c55e55"><h3>⚙️ تنظیمات به‌روزرسانی خودکار (از داخل اسکریپر)</h3><div class="note" style="margin-bottom:8px">بدون نیاز به متغیر محیطی — همین‌جا فعال/غیرفعال و فاصلهٔ بررسی را تعیین کنید. مقدار محیطی <code>SCRAPER_AUTO_UPDATE</code> در صورت تنظیم، بر این گزینه اولویت دارد.</div><div class="grid"><div><label>وضعیت</label><select id="auto_update_enabled" onchange="saveAutoUpdate()"><option value="1">فعال — هر N ثانیه بررسی</option><option value="0">غیرفعال — فقط دستی</option></select></div><div><label>فاصله بررسی (ثانیه، حداقل ۱۲۰)</label><input id="auto_update_interval" type="number" min="120" max="86400" value="300" onchange="saveAutoUpdate()"></div></div><div class="actions"><button onclick="saveAutoUpdate()">💾 ذخیره تنظیمات به‌روزرسانی</button><button class="gray" onclick="loadAutoUpdate()">↻ بارگذاری وضعیت</button></div><div id="autoUpdateStatus" class="status">—</div></div>
...
function scheduleProfileAutoSave(){ _debounceAuto('profile', async()=>{ try{ if(!activeProfile) return; const cfg=config(); await api('/api/profile',{method:'POST', body: JSON.stringify({name: activeProfile, config: cfg})}); const _s=$('status'); if(_s) _s.innerHTML='<span class="ok">✓ پروفایل «'+esc(activeProfile)+'» ذخیره خودکار شد</span>'; }catch(e){} }, 1200); }
let _profileAutoSaveInstalled=false;
function installProfileAutoSave(){ if(_profileAutoSaveInstalled) return; _profileAutoSaveInstalled=true; const ids=['url','pages','render','fetch_engine','fetch_engine_master','pagination','page_value','scrolls','enrich','detail_scope','detail_limit','sel_container','sel_title','sel_price','sel_link','sel_image','sel_sku','det_gallery','det_variations','det_weight','det_category','det_price','det_stock','det_brand','det_sku','det_short_desc','det_long_desc','det_tags','det_attributes','rule_title_prefix','rule_title_suffix','rule_price_mode','rule_price_value','rule_price_round','rule_default_stock','rule_default_category','rule_bsl_category_id','rule_woo_category_id','rule_woo_price_mode','rule_woo_price_value','rule_woo_price_round','rule_bsl_price_mode','rule_bsl_price_value','rule_bsl_price_round','galMode','galBox','galSelectors','galPattern','galFrom','galTo','galSkipFirst']; ids.forEach(id=>{ const el=document.getElementById(id); if(!el) return; const ev=(el.type==='checkbox'?'change':'input'); el.addEventListener(ev, scheduleProfileAutoSave); if(el.tagName==='SELECT') el.addEventListener('change', scheduleProfileAutoSave); }); }
...
let _savedScale=localStorage.getItem('scraperFontScale');if(_savedScale) applyFontScale(_savedScale,{save:false});else fetch('/api/appearance').then(r=>r.json()).then(d=>{if(d&&d.fontScale) applyFontScale(d.fontScale,{save:false});else applyFontScale(1,{save:false});}).catch(()=>applyFontScale(1,{save:false}));showAdmin('backupAdmin');
...
function loadVisualPicker(forceBrowser){
  ...
  else if($('pickerEngine')) mode=$('pickerEngine').value;
  else if(/snappshop\.ir|digikala\.com/i.test(url)) mode='browser';
  else mode=($('render')?.value||'auto');
  ...
  function _showPickerToast_OLD(msg, type){
    try{
      ...
      // also update status line for persistence
      $('pickerStatus').textContent=msg;
    }catch(e){}
  }
  // clear old timer
  if(window._pickerToastTimer) clearInterval(window._pickerToastTimer);
  let step=0;
  const isBrowser = mode==='browser';
  const steps = isBrowser ? [
    ['🔌 در حال اتصال به '+new URL(url).hostname+'…', 0],
    ['🎭 در حال رندر با '+(($('pickerEngine')?.value||'playwright')==='selenium'?'Selenium':'Playwright')+'…', 900],
    ['↕️ اسکرول صفحه و بارگذاری تنبل تصاویر…', 2200],
    ['⏳ انتظار کارت‌های محصول (Digikala/SPA)…', 3800],
    ['🧩 پردازش DOM و تزریق انتخابگر…', 5600]
  ] : [
    ['🔌 در حال دریافت HTTP…', 0],
    ['📄 دریافت HTML و پاکسازی اسکریپت‌ها…', 800],
...
window.addEventListener('message',event=>{
  if(event.source!==$('pickerFrame').contentWindow) return;
  let d=event.data||{};
  if(window._pickerToastTimer){ clearTimeout(window._pickerToastTimer); window._pickerToastTimer=null; }
  if(d.type==='s4-picker-ready'){
    pickerContext();
...
async function init(){loadTaskTopSummary();let d=await api('/api/config');currentBuild=d.build||'';$('appVersion').textContent='v'+(d.version||'10.123');profiles=d.profiles||{};$('timeout').value=d.network.timeout;$('gap_ms').value=d.network.gap_ms;$('proxy').value=d.network.proxy||'';$('proxy_mode').value=d.network.proxy_mode||'auto';$('worker_key').value=d.network.worker_key||'';$('verify_tls').checked=d.network.verify_tls!==false;updateGatewayUI();$('woo_url').value=d.woocommerce.url||'';$('woo_ck').value=d.woocommerce.consumer_key||'';$('woo_cs').value=d.woocommerce.consumer_secret||'';
  ...
  try{
    ...
    try{ installProfileAutoSave(); }catch(e){}
...
function openStoreManagerModal(){let modal=$('storeManagerModalClassic'),frame=$('storeManagerFrameClassic'),loading=$('storeManagerLoadingClassic');if(!modal||!frame)return;storeManagerClassicReturnFocus=document.activeElement;modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';setTimeout(()=>$('storeManagerCloseClassic')?.focus(),0);if(frame.dataset.ready==='1'){frame.hidden=false;if(loading)loading.hidden=true;return}if(loading){loading.hidden=false;loading.textContent='در حال بارگذاری پنل مدیریت فروشگاه…'}frame.hidden=true;frame.src=storeManagerClassicUrl(false)}
function closeStoreManagerModal(){let modal=$('storeManagerModalClassic');if(!modal)return;modal.classList.remove('open');modal.setAttribute('aria-hidden','true');if(!$('settingsPanel').classList.contains('open')&&!$('aiTestModal').classList.contains('open'))document.body.style.overflow='';if(storeManagerClassicReturnFocus&&typeof storeManagerClassicReturnFocus.focus==='function')storeManagerClassicReturnFocus.focus();storeManagerClassicReturnFocus=null}
function refreshStoreManagerModal(){let frame=$('storeManagerFrameClassic'),loading=$('storeManagerLoadingClassic');if(!frame)return;if(loading){loading.hidden=false;loading.textContent='در حال تازه‌سازی مدیریت فروشگاه…'}frame.hidden=true;frame.dataset.ready='';frame.src=storeManagerClassicUrl(true)}
function storeManagerFrameLoaded(){let frame=$('storeManagerFrameClassic'),loading=$('storeManagerLoadingClassic');if(!frame||!frame.src||frame.src==='about:blank')return;frame.dataset.ready='1';frame.hidden=false;if(loading)loading.hidden=true}
function storeManagerClassicMessage(event){let frame=$('storeManagerFrameClassic');if(event.origin===location.origin&&frame&&event.source===frame.contentWindow&&event.data?.type==='scraper4:close-store-manager')closeStoreManagerModal()}
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('storeManagerModalClassic')?.classList.contains('open'))closeStoreManagerModal()});
window.addEventListener('message',storeManagerClassicMessage);
...
async function loadAutoUpdate(){try{let d=await deployApi('/api/auto-update');$('auto_update_enabled').value=d.enabled?'1':'0';$('auto_update_interval').value=d.interval;let s=d.state||{};let last=d.enabled?('آخرین بررسی: '+(s.last?new Date(s.last*1000).toLocaleString('fa-IR'):'هرگز')+' · '+(s.error||'بدون خطا')+(d.env_overridden?' · (متغیر محیطی فعال است)':'')):'غیرفعال';$('autoUpdateStatus').innerHTML='<span class="'+(d.enabled?'ok':'')+'">'+(d.enabled?'فعال':'غیرفعال')+' · فاصله '+d.interval+' ثانیه · '+last+'</span>';}catch(e){$('autoUpdateStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>';}}
async function saveAutoUpdate(){try{let enabled=$('auto_update_enabled').value==='1',interval=parseInt($('auto_update_interval').value)||300;await deployApi('/api/auto-update',{method:'POST',body:JSON.stringify({enabled,interval})});$('autoUpdateStatus').innerHTML='<span class="ok">ذخیره شد</span>';showToast('✓ تنظیمات به‌روزرسانی ذخیره شد');setTimeout(loadAutoUpdate,800);}catch(e){$('autoUpdateStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>';}}
async function checkDeployerLocal(){try{$('deployerLocalStatus').textContent='در حال بررسی…';let d=await deployApi('/api/deployer/local-status');$('deployerLocalStatus').innerHTML=d.running?'<span class="ok">✓ دیپلویر محلی فعال است · پورت '+d.port+' · '+esc(d.path)+'</span>':'<span class="error">✗ دیپلویر در دسترس نیست: '+esc(d.error||'پورت بسته')+'</span>';}catch(e){$('deployerLocalStatus').innerHTML='<span class="error">'+esc(e.message)+'</span>';}}
async function loadPythonVersionInfo(){try{let r=await fetch('/health',{cache:'no-store'}),d=await r.json();let info='نسخه کد: <b>v'+esc(d.version||'?')+'</b> · پایتون '+esc(d.python||'?')+' · Flask '+esc(d.flask||'?')+' · '+esc(d.edition||'vps')+' · خودکار:'+(d.auto_update?'فعال':'غیرفعال');if(d.build)info+=' · BUILD '+esc(d.build.slice(0,8));$('pythonVersionInfo').innerHTML=info;}catch(e){$('pythonVersionInfo').textContent='—';}}
...
init().then(async()=>{setInterval(watchBuild,30000);setInterval(loadTaskTopSummary,10000);try{if($('dep_autocheck')&&$('dep_autocheck').checked){await deployCheck(false)}}catch(e){}try{loadAutoUpdate();loadPythonVersionInfo();checkDeployerLocal();}catch(e){}}).catch(e=>$('status').textContent=e.message);
</script></body></html>'''
