...
BRANCH="${BRANCH:-arena/01a0afa2-new}"
...
ensure_venv() {
  ...
  if ! "$PY" -c "import flask,gunicorn,requests,bs4,lxml" 2>/dev/null; then
    ...
    "$PY" -m pip install flask gunicorn requests beautifulsoup4 lxml html5lib || fail "pip install failed"
  fi
  ...
  for spec in "httpx[http2]" cloudscraper curl_cffi beautifulsoup4 lxml html5lib selenium undetected-chromedriver; do
    case "$spec" in
      httpx*) mod=httpx ;;
      *) mod="$spec" ;;
    esac
...
for n in ('bs4','lxml','html5lib','httpx','cloudscraper','curl_cffi','selenium'):
    try:
