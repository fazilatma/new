...
test('real Node HTTP extraction and sync buttons drain SQLite with continuous worker disabled', {skip:!sqliteAvailable,timeout:60000}, async()=>{
  ...
  await build({entryPoints:[new URL('../render-src/server.ts',import.meta.url).pathname],outfile,bundle:true,platform:'node',format:'esm',packages:'external',logLevel:'silent',plugins:[{name:'offline-source',setup(b){
    ...
    b.onLoad({filter:/.*/,namespace:'fixture'},args=>({contents:args.path==='delivery'?`import {writeFile} from 'node:fs/promises';export const syncWoo=async product=>{await writeFile(${JSON.stringify(sentFile)},JSON.stringify(product));return 'updated'};export const syncBasalam=async()=>[];`:args.path==='categories'?'export const ledgerMissing=async()=>({planned:0,changed:0});export const destinationCategories=async()=>({items:[{id:17,name:"Shoes"}]});':`export const safeText=async url=>({url,text:${JSON.stringify(fixture)},status:200});export const sourceRoute=async()=>({mode:'direct'});export const assertPublicUrl=async()=>{};`}));
  }}]});
  ...
  try{
    ...
    // Backend extraction is a durable list-only workflow, including after retry.
    const listOnly=await api(`/api/profiles/${profile.id}/scrape`,{target:'woo',workflow:'list-only'});
    for(let i=0;i<100;i++){({job}=await api('/api/jobs/'+listOnly.job.id));if(!['queued','running'].includes(job.status))break;await delay(50)}
    assert.equal(job.status,'done',JSON.stringify(job));assert.equal(job.workflow,'list-only');assert.equal(job.target,'none');
    assert.ok(!job.log.some(r=>r.level==='stage'&&['details','ai-descriptions','basalam-categories','sync'].includes(r.message)));
    const listed=await api(`/api/profiles/${profile.id}/products`);assert.equal(listed.products[0].basalamCategoryId,17,'list-only must preserve the existing category without running categorization');
    await api('/api/jobs/'+job.id+'/retry',{});
    for(let i=0;i<100;i++){({job}=await api('/api/jobs/'+listOnly.job.id));if(!['queued','running'].includes(job.status))break;await delay(50)}
    assert.equal(job.workflow,'list-only');assert.equal(job.status,'done');
    const full=await api(`/api/profiles/${profile.id}/scrape`,{target:'woo',workflow:'full'});
    for(let i=0;i<100;i++){({job}=await api('/api/jobs/'+full.job.id));if(!['queued','running'].includes(job.status))break;await delay(50)}
    assert.equal(job.workflow,'full');assert.equal(job.status,'done',JSON.stringify(job));assert.ok(job.log.some(r=>r.level==='stage'&&r.message==='sync'));
    assert.equal((await api(`/api/profiles/${profile.id}/products`)).products[0].basalamCategoryId,17);
