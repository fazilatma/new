...
const mockFetch = async (input, init = {}) => {
  ...
  if (url.pathname === '/api/status') return json({ ok: true, version: '1.172.0', databaseReady: true, connections: { woo: { ok: false }, basalam: { ok: false } } });
  if (url.pathname === '/health') return json({ ok: true, version: '1.172.0', databaseReady: true });
  ...
  if (url.pathname === '/api/deployer/branches') {
    ...
    return json({ ok: true, repo, running: '1.172.0', cached: false, latest: 'arena/01a09468-new', branches: [
      { name: 'main', version: '1.160.0', status: 'older' },
      { name: 'arena/01a09468-new', version: '1.172.0', status: 'equal' }
    ] });
  }
