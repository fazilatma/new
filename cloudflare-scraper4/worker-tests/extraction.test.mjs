...
test('worker: zero extracted products triggers the auto-suggest fallback', () => {
  const source = stripComments(workerProcessor);
  assert.match(source, /if\(!selectedProductParser\(profile\)&&listSelectorsStatus\(profile\.selectors\)!=='custom'&&!page\.products\.length&&!checkpoint\.listRescued\)/,
    'the worker must rescue an empty page before throwing');
  const branch = source.slice(source.indexOf("if(!selectedProductParser(profile)&&listSelectorsStatus(profile.selectors)!=='custom'&&!page.products.length&&!checkpoint.listRescued)"));
