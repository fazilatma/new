...
class MemoryStatement {
  ...
  async run(){const s=this.sql,v=this.values;
    ...
    else if(s.includes("SET status='queued'")&&s.includes("status IN ('failed','stopped')")&&s.includes("status='running' AND updated_at<?")){let n=0;for(const job of this.db.jobs.values())if(['failed','stopped'].includes(job.status)||(job.status==='running'&&String(job.updated_at||'')<String(v[1]))){job.status='queued';job.phase='waiting';job.stop_requested=0;job.error=null;job.finished_at=null;job.updated_at=v[0];n++}return{success:true,meta:{changes:n}}}
