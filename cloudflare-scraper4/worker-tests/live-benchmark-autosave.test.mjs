...
for(const runtime of ['render','worker'])for(const enabled of [false,true])for(const withDetails of [false,true])test(runtime+' parser='+enabled+' details='+withDetails+': three-page benchmark emits real progress before the first page completes',async()=>{
 ...
 let release,calls=0,detailCalls=0;const events=[],saved=[];
 ...
 const io={extractDiagnosticSample:async product=>{detailCalls++;return {product:{...product,longDesc:'Full description'}}},...await compile(await read('worker-src/diagnostic-details.ts'),['diagnosticDetails'],{benchmarkError:e=>e.message}),...await compile(await read('worker-src/benchmark-evidence.ts'),['benchmarkEvidence','incompatibleBenchmark']),...await compile(await read('worker-src/product-parser.ts'),['selectedProductParser']),...await compile(await read('worker-src/benchmark-pagination.ts'),['benchmarkPagination']),BENCHMARK_ENGINES:['cheerio','missing',...(enabled?['network_api']:[])],MIN_BENCHMARK_PRODUCTS:2,BROWSER_ENGINES:new Set(['missing']),WORKER_UNAVAILABLE_ENGINES:new Set(['missing']),browserEngineAvailable:()=>false,benchmarkProbeUrl:p=>p.url,pageUrl:(p,n)=>p.url+'?page='+n,safeText:async()=>({text:'fixture',url:'https://shop.test'}),sourceText:async()=>({text:'fixture',url:'https://shop.test'}),scrapeListWithMeta:scrape,scrapeListPage:scrape,diagnoseBenchmarkEngine:async()=>({hint:'fixture'}),saveBenchmarkProfile:async(...args)=>{saved.push(args);return true},message:e=>e.message};
 const {benchmarkProfileEngines}=await compile(src.slice(a,b),['benchmarkProfileEngines'],io);
 const pending=benchmarkProfileEngines({id:'p',url:'https://shop.test',selectors:{},pagination:'query_page',productParserEnabled:enabled,productParser:'jsonld'},e=>events.push(e),withDetails);await tick();assert.equal(calls,1);assert.ok(events.some(e=>e.page===1&&e.status==='running'));assert.equal(saved.length,0);
 release();const result=await pending;assert.equal(calls,3);assert.equal(detailCalls,withDetails?1:0);assert.equal(result.fastest.sample.longDesc,withDetails?'Full description':undefined);assert.equal(result.fastest.products,6);assert.equal(result.fastest.sample.title,'Actual product 1');assert.equal(result.fastest.diagnosis.complete.title,6);assert.equal(result.fastest.diagnosis.complete.image,3);if(enabled){const skip=result.results.find(r=>r.engine==='network_api');assert.equal(skip.status,'incompatible');assert.equal(skip.sample,null);assert.ok(events.some(e=>e.name==='network_api'&&e.status==='skipped'));}assert.ok(events.some(e=>e.page===3&&e.pagesScanned===3));assert.ok(events.some(e=>e.name==='missing'&&e.status==='skipped'));assert.equal(saved.length,enabled?0:1);if(enabled){assert.equal(result.profileUpdated,false);assert.equal(result.productParser,'jsonld');}
});
