...
const ledgerCore=await compileFunctions(await read('worker-src/destination-ledger.ts'),['digest','LEDGER_TTL']);
const listHelpers=await compileFunctions(await read('worker-src/source-list-ledger.ts'),['reuseSourceList','sourceDetailFailed','mergeListOnly'],ledgerCore);
...
for(const runtime of ['worker','render']){
 test(runtime+': backend list-only never invokes enrichment or delivery, even with a send target',async()=>{
  const run=await pipeline(runtime,{workflow:'list-only',target:'woo'});
  assert.equal(run.job.target,'none');assert.equal(run.saved.length,1);assert.equal(run.syncs.length,0);assert.equal(run.events.length,0);
  assert.equal(run.saved[0].basalamCategoryId,undefined);assert.ok(!run.job.log.some(r=>r.level==='stage'&&['details','details-save-sync','basalam-categories','ai-descriptions','sync','sync-woo'].includes(r.message)));
 });
 test(runtime+': native category prediction wins before learned/model fallback, but never overwrites manual selection',async()=>{
  let calls=0;const ai=await aiHarness(runtime,{loadConnections:async()=>({basalam:{token:'fixture-token'}}),getState:async()=>null,setState:async()=>{},safeFetch:async()=>{},safeBasalamFetch:async()=>{},predictBasalamCategory:async()=>{calls++;return{ok:true,categoryId:42}}});
  const p=product();assert.equal((await ai.assignProductBasalamCategory(p,{categories})).ok,true);assert.equal(p.basalamCategoryId,42);assert.equal(p.basalamCategorySource,'basalam-prediction');assert.equal(ai.events.length,0);
  await ai.assignProductBasalamCategory(p,{categories});assert.equal(calls,1);
  await ai.assignProductBasalamCategory(product(),{categories,profileCategoryId:42});assert.equal(calls,1);
 });
 test(runtime+': failed native prediction falls back to the existing model path',async()=>{
  const ai=await aiHarness(runtime,{loadConnections:async()=>({basalam:{token:'fixture-token'}}),getState:async()=>null,setState:async()=>{},safeFetch:async()=>{},safeBasalamFetch:async()=>{},predictBasalamCategory:async()=>({ok:false,error:'timeout'})});
