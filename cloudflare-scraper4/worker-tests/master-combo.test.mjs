...
const mockFetch = async (input, init = {}) => {
  ...
  if (url.pathname === '/api/status') return json({ ok: true, version: '1.163.0', databaseReady: true, connections: { woo: { ok: false }, basalam: { ok: false } } });
  if (url.pathname === '/health') return json({ ok: true, version: '1.163.0', databaseReady: true });
