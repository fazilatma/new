...
test('dashboard JavaScript parses and required element references exist',async()=>{const source=await read('worker-src/dashboard.ts'),match=source.match(/export const DASHBOARD_JS = String\.raw`\n([\s\S]*?)\n`;/);assert.ok(match);new vm.Script(match[1]);const refs=[...match[1].matchAll(/\$\('([^']+)'\)/g)].map(x=>x[1]);const dynamic=new Set([...match[1].matchAll(/m(?:Select|Input|Check)\('[^']*','([^']+)'/g),...match[1].matchAll(/mFile\('([^']+)'/g),...match[1].matchAll(/\.id='([^']+)'/g)].map(x=>x[1]));const known=new Set('sel-variations autoSaveState aiProgressSinceLast aiProgressAvg aiProgressCurrentMs aiProgressEta aiProgressRetry bsShopVid bsShopName bulkProfile bulkQuery bulkPrefix bulkSuffix bulkPrice bulkStock bulkTarget destProductId destStatus catId catName'.split(' '));const missing=[...new Set(refs)].filter(id=>!dynamic.has(id)&&!source.includes(`id=\\"${id}\\"`)&&!source.includes(`id="${id}"`));assert.deepEqual(new Set(missing),known)});
