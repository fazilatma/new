...
async function wooUpdate(id:number,payload:any){const c=(await loadConnections()).woo,auth=`Basic ${Buffer.from(`${c.key}:${c.secret}`).toString('base64')}`,r=await safeFetch(`${c.url}/wp-json/wc/v3/products/${id}`,{method:'PUT',headers:{authorization:auth,'content-type':'application/json'},body:JSON.stringify(payload),apiMode:true,directRoute:true},3_000_000);if(!r.ok)throw Error(`Woo update ${id}: HTTP ${r.status}`)}
