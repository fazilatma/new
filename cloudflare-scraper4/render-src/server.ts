...
const PACKAGE_VERSION = (() => { try { return JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version || '1.162.0'; } catch { return process.env.npm_package_version || '1.162.0'; } })();
...
app.post('/api/destination/basalam/category-runs',async c=>{try{const started=await startCategoryRun();return c.json({ok:true,...started},started.existing?200:202)}catch(error){return c.json({ok:false,error:error instanceof Error?error.message:String(error)},400)}});
