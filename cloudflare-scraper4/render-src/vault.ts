...
export function mergeConnections(base: ConnectionVault, input: any): ConnectionVault {
  ...
  const providers=Array.isArray(providerInput)?providerInput.map((p:any,i:number)=>({id:text(p?.id)||`provider-${i+1}`,name:text(p?.name)||text(p?.id)||`Provider ${i+1}`,baseUrl:text(p?.baseUrl||p?.base_url||p?.url).replace(/\/$/,''),apiKey:text(p?.apiKey||p?.api_key),models:Array.isArray(p?.models)?p.models.map((model:any)=>typeof model==='string'?model:String(model?.id||model?.name||'')).filter(Boolean):[],apiKeys:Array.isArray(p?.apiKeys)?p.apiKeys.map((k:any)=>typeof k==='string'?k:text(k?.key||k?.token)).filter(Boolean):[],reasoningModels:Array.isArray(p?.reasoningModels)?p.reasoningModels.map(String):[],nonChatModels:Array.isArray(p?.nonChatModels)?p.nonChatModels.map(String):[],...(p?.vendor?{vendor:text(p.vendor)}:{}),enabled:p?.enabled!==false})):base.ai.providers;
