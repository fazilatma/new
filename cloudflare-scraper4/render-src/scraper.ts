...
function metadataProduct(html: string, baseUrl: string): Product[] { const title = meta(html, 'og:title') || meta(html, 'twitter:title') || stripHtml(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || ''); if (!title) return []; const productUrl = absolute(meta(html, 'og:url') || baseUrl, baseUrl); const priceText = meta(html, 'product:price:amount') || meta(html, 'og:price:amount') || ''; const image = absolute(meta(html, 'og:image') || meta(html, 'twitter:image'), baseUrl); return [{ sourceKey: sourceKey(productUrl, title), title, price: numberFromText(priceText), priceText, url: productUrl, image, images: image ? [image] : [], sourcePage: baseUrl, scrapedAt: new Date().toISOString() }]; }
