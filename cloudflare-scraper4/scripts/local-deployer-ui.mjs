...
function autoUpdateFromGit({ branch, reason = 'timer', force = true, install = true } = {}) {
  ...
  const dirtyProbe = runSync('git', ['status', '--porcelain']);
  if (dirtyProbe.ok && dirtyProbe.stdout.trim()) {
    const lockOnly = dirtyProbe.stdout.trim().split('\n')
      .every(line => /\s(?:cloudflare-scraper4\/)?package-lock\.json$/.test(line));
    if (lockOnly) runSync('git', ['checkout', '--', 'package-lock.json']);
  }
...
async function proxyScraper(req, res, url) {
  ...
  upstream.on('error', error => send(res, 502, { ok: false, error: `Local scraper proxy failed: ${error.message}`, port: scraperPort, hint: 'The scraper answered the readiness probe but dropped this request; check the Scraper tab log.' }));
  upstream.end(body);
}
