...
function updateFromGit({ branch, force = false, install = false } = {}) {
  ...
  if (install && changed && steps.at(-1).ok) {
    ...
    if(steps.at(-1).ok&&isTermuxInstall){try{startBrowserInstall({cwd:projectDir});}catch(error){console.warn('Optional browser setup: '+error.message);}}
...
function runJob(name, command, args = [], options = {}) {
  ...
  child.on('exit', code => { if(name==='install'&&code===0&&isTermuxInstall){try{startBrowserInstall({cwd:projectDir});}catch(error){appendLog(name,'Optional browser setup: '+error.message);}} job.running = false; job.exitCode = code ?? 0; job.finishedAt = new Date().toISOString(); appendLog(name, `\n[process exited with code ${job.exitCode}]\n`); });
  return job;
}
