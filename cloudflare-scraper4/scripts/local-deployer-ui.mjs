...
function autoUpdateFromGit({ branch, reason = 'timer', force = true, install = true } = {}) {
  ...
  if (dirtyProbe.ok && dirtyProbe.stdout.trim()) {
    ...
    if (lockOnly) {
      const lockPaths = dirtyProbe.stdout.split('\n')
        .map(line => (line.match(/^..\s+(.*)$/) || [])[1] || '')
        .map(path => path.trim().replace(/^"|"$/g, ''))
        .filter(path => /package-lock\.json$/.test(path));
      if (lockPaths.length) runSync('git', ['checkout', '--', ...lockPaths]);
    }
  }
