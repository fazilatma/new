...
import { createResourceMonitor } from './deployer-resources.mjs';
...
function resourcePath(samples,field){
  let drawing=false;return samples.map(function(sample,index){const value=field(sample);if(!Number.isFinite(value)){drawing=false;return '';}
    const x=samples.length>1?index/(samples.length-1)*600:0,y=100-Math.max(0,Math.min(100,value));
