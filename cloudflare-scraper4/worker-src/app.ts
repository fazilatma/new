...
import {normalizeProductParser,selectedProductParser} from './product-parser.js';
import { benchmarkPagination } from './benchmark-pagination.js';
import { extractionDetails } from './job-details.js';
import { refreshDestinationLedger, destinationLedgerStatus, destinationLedgerProducts, ledgerMissing } from './maintenance.js';
import { maintenanceResponse } from './maintenance-response.js';
...
import { benchmarkProbeUrl, diagnoseBenchmarkEngine, diagnoseExtraction, mapLimit, numberFromText, pageUrl, scrapeDetails, scrapeListPage, sourceText, suggestSelectors, testGallery, testSelector, testVariations } from './scraper.js';
...
app.get('/health',c=>c.json({ok:true,app:'scraper4-cloudflare',runtime:'cloudflare-workers',databaseReady:Boolean(c.env.DB),databaseError:c.env.DB?null:'D1 binding DB is missing',workerInWeb:Boolean(c.env.JOBS),authenticationRequired:false,version:c.env.WORKER_VERSION||'1.227.0+',time:new Date().toISOString()}));
...
app.get('/api/activity',async c=>{
  const[profiles,jobs,aiRun,dedupRun,catRun,agentRun,cronLock,priorities,runPriorities,version,allActive,operations]=await Promise.all([
    ...
    getJobPriorities(),
    getRunPriorities(),
    Promise.resolve(c.env.WORKER_VERSION||'1.227.0+'),listActiveJobs(),listLiveActivities()
  ]);
...
app.get('/api/version',c=>c.json({ok:true,version:c.env.WORKER_VERSION||'1.227.0+',runtime:'cloudflare-workers',deployment:'wrangler versions deploy / wrangler rollback'}));
...
// Runtime parity for the dashboard's «دیپلویر» section: a Worker has no local deployer to
// proxy, and must not 404 on it. One clear answer keeps the panel's copy identical in both
// runtimes instead of failing with a generic fetch error.
app.on(['GET', 'POST'], '/api/deployer/local/:action', async c => c.json({
  ok: false,
  code: 'NO_DEPLOYER',
  error: 'این رانتایم دیپلویر محلی ندارد؛ دیپلویر روی همان دستگاهی اجرا می‌شود که کد را نصب می‌کند.',
  hint: 'روی Cloudflare و Render انتشار با push به شاخهٔ production انجام می‌شود؛ جدول برنچ‌ها در همین سرور کار می‌کند.'
}, 501));

app.post('/api/deployer/install-branch',async c=>c.json({ok:false,code:'NO_DEPLOYER',error:'Branch install needs a deployer-managed Node scraper; this Worker cannot install code.'}));
...
app.on(['GET','POST'],'/api/runtime/browser-session',c=>c.json({ok:false,supported:false,error:'آزمون مرورگر پایدار فقط روی Node (VPS/Termux) ممکن است؛ Cloudflare Worker مرورگر محلی ندارد.'},400));
app.get('/api/runtime/browser-session/report',c=>c.json({ok:false,supported:false,error:'گزارش مرورگر پایدار را از اجرای Node بگیرید.'},400));
...
app.get('/api/ai/description-settings',async c=>{
  const settings=await getState<any>('ai_description_settings',{enabled:true}),picked=await preferredAiChatModel(),last=await getState<any>(AI_ENRICH_LAST_KEY,null);
...
app.post('/api/profiles/:id/ai-descriptions',async c=>{
  ...
  const targets=stored.filter((row:any)=>force||productNeedsEnrichment(row).any||productNeedsBasalamCategory(row)).slice(0,limit);
...
app.post('/api/notifications/test',async c=>{const b=await jsonBody(c);return c.json(await sendNotification(b.channel||'webhook',String(b.text||'پیام آزمایشی اسکرپر ۴')))});
...
app.post('/api/maintenance/recon-unified',async c=>{const b=await jsonBody(c);return maintenanceResponse(c,()=>unifiedRecon(String(b.profileId||'')))});
app.post('/api/maintenance/recon-unified/apply',async c=>{const b=await jsonBody(c);return maintenanceResponse(c,()=>unifiedReconApply(String(b.profileId||''),b.confirm==='APPLY',Number(b.limit)||200))});
// Request 36b: preview (no confirm) or delete duplicates in every destination,
// keeping the most expensive copy by default.
app.post('/api/maintenance/duplicates',async c=>{const b=await jsonBody(c);return maintenanceResponse(c,()=>destinationDuplicates(b.confirm==='APPLY',Number(b.limit)||200,b.keep==='cheapest'?'cheapest':'expensive',String(b.accountKey||'')))});
...
app.post('/api/source-test',async c=>{const b=await jsonBody(c),profile=b.profileId?await getProfile(String(b.profileId)):null,result=await sourceText(String(b.url||''),Boolean(profile?.networkIndirect),1_000_000);return c.json({ok:true,bytes:byteLength(result.text),url:result.url,route:result.route,title:(result.text.match(/<title[^>]*>(.*?)<\/title>/is)?.[1]||'').replace(/<[^>]+>/g,'').trim()})});
...
async function benchmarkProfileEngines(profile:Profile,onProgress?:DiagnosticObserver,withDetails=false){
  ...
  for(const engine of BENCHMARK_ENGINES){
    ...
    paginationReport=await benchmarkPagination(probe,{pageUrl,
      scrape:(url,nextSelector)=>scrapeListPage(url,profile.selectors,nextSelector,Boolean(profile.networkIndirect),engine,undefined,false,true,false,selectedProductParser(profile)),
  ...
  return{ok:Boolean(fastest),...(selectedProductParser(profile)?{productParser:selectedProductParser(profile)}:{}),profileUpdated,profileId:profile.id,startedAt,pages,pagination:profile.pagination,fastest,results,discoveredSelectors:benchmarkDiscovered,recommendations:selectedProductParser(profile)?['پارسر مرحلهٔ دوم: '+selectedProductParser(profile)+'؛ مقایسهٔ دریافت صفحه بدون تغییر تنظیمات ذخیره‌شده انجام شد.']:fastest?[`بهترین موتور به‌عنوان پیش‌فرض پروفایل ذخیره شد: ${fastest.engine} (${fastest.products} محصول در ${fastest.pagesScanned} صفحه/دسته).`]:(best&&best.products>0?[`هیچ موتوری به اندازهٔ کافی محصول پیدا نکرد (بیشترین: ${best.products}). موتور پیش‌فرض پروفایل تغییر نکرد.`]:['هیچ موتوری در سه صفحهٔ اول محصولی استخراج نکرد. دسترسی شبکه، پاسخ ضدربات و سلکتورها را بررسی کنید.','موتور پیش‌فرض پروفایل بدون تغییر باقی ماند.'])};
}

app.post('/api/profiles/:id/extraction-diagnostic',async c=>{const profile=await getProfile(c.req.param('id'));if(!profile)return c.json({ok:false,error:'پروفایل پیدا نشد.'},404);const b=await jsonBody(c);const run=async(onProgress?:DiagnosticObserver)=>{const report:any=await diagnoseExtraction(profile,String(b.url||''),onProgress,b.withDetails===true);const toSave=report.selectorsToSave||{},keys=Object.keys(toSave).filter(key=>String(toSave[key]||'').trim());if(keys.length){onProgress?.({name:'selectors-auto-saved',status:'running',summary:'در حال ذخیرهٔ سلکتورهای پیدا‌شده در پروفایل…',count:keys.length});const selectors={...profile.selectors}as any;for(const key of keys)selectors[key]=toSave[key];await saveProfile({...profile,selectors,updatedAt:new Date().toISOString()});report.selectorsSaved=Object.fromEntries(keys.map(key=>[key,toSave[key]]));report.stages.push({name:'selectors-auto-saved',ok:true,summary:'سلکتورهای پیداشده به‌صورت خودکار در تب سلکتورها ذخیره شدند.',selectors:report.selectorsSaved});onProgress?.({...report.stages[report.stages.length-1],status:'success'})}else onProgress?.({name:'selectors-auto-saved',status:'skipped',summary:'سلکتور تازه‌ای برای ذخیره وجود ندارد.'});return report};if(c.req.query('live')==='1')return diagnosticStream(run);return c.json(await run())});
app.post('/api/profiles/:id/benchmark-engines',async c=>{const profile=await getProfile(c.req.param('id'));if(!profile)return c.json({ok:false,error:'پروفایل پیدا نشد.'},404);const options=await c.req.json().catch(()=>({}))as any;if(c.req.query('live')==='1')return diagnosticStream(observe=>benchmarkProfileEngines(profile,observe,options.withDetails===true));return c.json(await benchmarkProfileEngines(profile,undefined,options.withDetails===true))});
app.post('/api/test-selector',async c=>{const b=await jsonBody(c);if(b.type==='variations')return c.json({ok:true,...await testVariations(String(b.url||''),String(b.selector||''),String(b.engine||''))});if(b.type==='gallery')return c.json({ok:true,...await testGallery(String(b.url||''),String(b.selector||''),Number(b.max)||30,Boolean(b.skipFirst),String(b.engine||''))});return c.json({ok:true,...await testSelector(String(b.url||''),String(b.selector||''),String(b.type||'text'),String(b.engine||''))})});
app.post('/api/suggest-selectors',async c=>{const b=await jsonBody(c),mode=['list','detail'].includes(b.mode)?b.mode:'all';return c.json({ok:true,...await suggestSelectors(String(b.url||''),mode,String(b.engine||''))})});
...
app.post('/api/profiles',async c=>{
 ...
 if(before&&['priceMode','priceValue','roundPrice'].some(key=>String((before as any)[key]??'')!==String((saved as any)[key]??''))){
  ...
  if(target!=='none'){job=await createJob(saved.id,'sync',target,{priceSync:true});if(job)await enqueueJob(job,p=>c.executionCtx.waitUntil(p));}
 }
...
app.delete('/api/profiles/:id/products',async c=>{if(c.req.query('confirm')!=='DELETE')return c.json({ok:false,error:'برای حذف همهٔ نتایج، confirm=DELETE لازم است.'},400);return c.json({ok:true,deleted:await clearProducts(c.req.param('id'))})});
...
app.get('/api/jobs',async c=>c.json({ok:true,jobs:await listJobs(Math.min(200,Number(c.req.query('limit'))||50)),processor:{mode:c.env.JOBS?'cloudflare-queue':'inline'}}));
...
async function connectionDiagnostic(target:string,input:any){
  ...
  try{
    ...
    if(target==='ai'){
      ...
      const provider=transient||providers.find(x=>x.id===input?.provider)||providers.find(x=>x.enabled&&x.models.length),model=String(input?.model||provider?.models[0]||''),prompt=String(input?.prompt||'Reply with exactly: SCRAPER4_OK');
...
async function runProfileApi(c:any,id:string){
  ...
  if(extract){
    for(let pageNo=1;pageNo<=pages&&products.length<limit;pageNo++)try{
      const url=pageUrl(profile,pageNo),page=await scrapeListPage(url,profile.selectors,profile.pagination==='next_selector'?profile.paginationValue:'',Boolean(profile.networkIndirect),profile.extractionEngine,profile.extractionEngineMaster,true,true,profile.pagination==='scroll',selectedProductParser(profile));
      pagesScanned++;usedEngine=page.usedEngine||usedEngine;engineMs+=page.elapsedMs||0;
      if(!selectedProductParser(profile)&&page.usedEngine&&page.products.length&&(profile.extractionEngine==='auto'||profile.extractionEngineMaster!==page.usedEngine)){profile.extractionEngineMaster=page.usedEngine;profile.extractionEngineHost=new URL(page.url).hostname;profile.extractionEngineMs=page.elapsedMs||0;await saveProfile({...profile,updatedAt:new Date().toISOString()})}
      if(!selectedProductParser(profile)&&page.discoveredSelectors&&Object.keys(page.discoveredSelectors).length){profile.selectors={...profile.selectors,...page.discoveredSelectors};await saveProfile({...profile,updatedAt:new Date().toISOString()});Object.assign(engineDiscovered,page.discoveredSelectors)}if(!selectedProductParser(profile)&&page.usedEngine&&page.products.length&&!isManualListEngine(page.usedEngine)){autoSelectorsAllowed=true;listSelectorUpdate=await applyInlineSelectorSuggestions(profile,page.url,'list',errors,true)}
      const before=products.length;
      for(const raw of page.products){const product=raw;if(seen.has(product.sourceKey))continue;seen.add(product.sourceKey);products.push(product);if(products.length>=limit)break}
    ...
    if(persist)for(const product of products)try{(await upsertProduct(profile.id,product,{source:true}))==='added'?added++:updated++}catch(error){errors.push(`${product?.title||'?'}: save: ${message(error)}`)}
...
async function createProfileJob(c:any,id:string,kind:'scrape'|'sync'){const profile=await getProfile(id);if(!profile)return c.json({ok:false,error:'Profile not found'},404);const b=await c.req.json().catch(()=>({})),target=validTarget(b.target||(kind==='sync'?'both':'none')),job=await createJob(profile.id,kind,target,{workflow:b.workflow==='list-only'?'list-only':b.workflow==='full'?'full':undefined});if(job.status==='queued')await enqueueJob(job,(promise:Promise<unknown>)=>c.executionCtx.waitUntil(promise));return c.json({ok:true,job,processor:job.kind===kind&&job.status==='queued'?'triggered':'existing-active',dedupProfile:true},202)}
...
export function normalizeProfile(raw:any):Profile {
  ...
  const master=(['cheerio','htmlrewriter','jsonld','next_data','metadata','script_json','heuristic','structural','playwright','puppeteer','crawlee_playwright','network_api'].includes(rawMaster)?rawMaster:undefined);
  ...
  return {
    ...
    aiDescriptions:raw.aiDescriptions===undefined?true:on(raw.aiDescriptions),
...
export async function scheduledTasks(env:Env,waitUntil:(promise:Promise<unknown>)=>void):Promise<void>{
  ...
  try{
    ...
    await refreshDestinationLedger(false);
    ...
    waitUntil(scheduledBranchPushTick({settings,envToken:env.GH_BACKUP_TOKEN,loadLast:()=>getState<any>('branch_push_last',null),saveLast:rec=>setState('branch_push_last',rec),buildBundle:()=>createPhpSettingsBundle(),connect:token=>({getter:githubApiFetch(token),putter:githubApiPut(token)}),snapshotDatabase:async()=>({skipped:'d1'}),log:m=>console.log('[scheduled-push]',m)}));
