...
app.get('/health',c=>c.json({ok:true,app:'scraper4-cloudflare',runtime:'cloudflare-workers',databaseReady:Boolean(c.env.DB),databaseError:c.env.DB?null:'D1 binding DB is missing',workerInWeb:Boolean(c.env.JOBS),authenticationRequired:false,version:c.env.WORKER_VERSION||'1.232.0+',time:new Date().toISOString()}));
...
app.get('/api/activity',async c=>{
  const[profiles,jobs,aiRun,dedupRun,catRun,agentRun,cronLock,priorities,runPriorities,version,allActive,operations]=await Promise.all([
    ...
    getJobPriorities(),
    getRunPriorities(),
    Promise.resolve(c.env.WORKER_VERSION||'1.232.0+'),listActiveJobs(),listLiveActivities()
  ]);
...
app.get('/api/version',c=>c.json({ok:true,version:c.env.WORKER_VERSION||'1.232.0+',runtime:'cloudflare-workers',deployment:'wrangler versions deploy / wrangler rollback'}));
...
app.post('/api/profiles/:id/extraction-diagnostic',async c=>{const profile=await getProfile(c.req.param('id'));if(!profile)return c.json({ok:false,error:'پروفایل پیدا نشد.'},404);const b=await jsonBody(c);const run=async(onProgress?:DiagnosticObserver)=>{const report:any=await diagnoseExtraction(profile,String(b.url||''),onProgress,b.withDetails===true);const toSave=report.selectorsToSave||{},keys=Object.keys(toSave).filter(key=>String(toSave[key]||'').trim());if(keys.length){onProgress?.({name:'selectors-auto-saved',status:'running',summary:'در حال ذخیرهٔ سلکتورهای پیدا‌شده در پروفایل…',count:keys.length});const selectors={...profile.selectors}as any;for(const key of keys)selectors[key]=toSave[key];await saveLearnedProfile(profile,{...profile,selectors},toSave);const stored=await getProfile(profile.id);report.selectorsSaved=Object.fromEntries(keys.filter(key=>(stored?.selectors as any)?.[key]===toSave[key]).map(key=>[key,toSave[key]]));report.stages.push({name:'selectors-auto-saved',ok:true,summary:Object.keys(report.selectorsSaved).length?'سلکتورهای مجاز بدون بازنویسی ویرایش دستی ذخیره شدند.':'پروفایل تغییر کرده است؛ سلکتورهای دستی حفظ شدند و چیزی بازنویسی نشد.',selectors:report.selectorsSaved});onProgress?.({...report.stages[report.stages.length-1],status:'success'})}else onProgress?.({name:'selectors-auto-saved',status:'skipped',summary:'سلکتور تازه‌ای برای ذخیره وجود ندارد.'});return report};if(c.req.query('live')==='1')return diagnosticStream(run);return c.json(await run())});
