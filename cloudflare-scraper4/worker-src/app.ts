...
app.get('/health',c=>c.json({ok:true,app:'scraper4-cloudflare',runtime:'cloudflare-workers',databaseReady:Boolean(c.env.DB),databaseError:c.env.DB?null:'D1 binding DB is missing',workerInWeb:Boolean(c.env.JOBS),authenticationRequired:false,version:c.env.WORKER_VERSION||'1.178.0',time:new Date().toISOString()}));
...
app.get('/api/activity',async c=>{
  const[profiles,jobs,aiRun,dedupRun,catRun,agentRun,cronLock,priorities,runPriorities,version]=await Promise.all([
    ...
    getJobPriorities(),
    getRunPriorities(),
    Promise.resolve(c.env.WORKER_VERSION||'1.178.0')
  ]);
...
app.get('/api/version',c=>c.json({ok:true,version:c.env.WORKER_VERSION||'1.178.0',runtime:'cloudflare-workers',deployment:'wrangler versions deploy / wrangler rollback'}));
...
export async function scheduledTasks(env:Env,waitUntil:(promise:Promise<unknown>)=>void):Promise<void>{
  ...
  try{
    ...
    waitUntil(aiEnrichTick({enabled:async()=>(await getState<any>('ai_description_settings',{enabled:true}))?.enabled!==false,modelReady:async()=>Boolean(await preferredAiChatModel()),listProfileIds:async()=>(await listProfiles()).map(p=>p.id),loadCursor:()=>getState<any>(AI_ENRICH_LAST_KEY,null),saveCursor:rec=>setState(AI_ENRICH_LAST_KEY,rec),listStalest:(profileId,limit)=>listStalestProducts(profileId,limit),enrich:product=>generateProductDescription(product),saveProduct:(profileId,product)=>upsertProduct(profileId,product as any),log:m=>console.log('[ai-enrich]',m)}));
