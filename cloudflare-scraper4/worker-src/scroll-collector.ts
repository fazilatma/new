...
export async function collectScrollProducts<T>(io:{snapshot():Promise<T[]>;step():Promise<ScrollState>;key(product:T):string;wait(ms:number):Promise<void>;now():number;stopped?():Promise<boolean>;observe?(products:T[],added:number,round:number):void},limits:{quietMs?:number;timeoutMs?:number;intervalMs?:number;maxRounds?:number;maxProducts?:number;maxBatches?:number}={}):Promise<T[]>{
 ...
 const found=new Map<string,T>(),start=io.now();let quietSince=io.now(),lastGeometry='',lastVisible='',batches=0;
 for(let round=0;round<maxRounds;round++){
  ...
  io.observe?.(products,found.size-before,round);if(found.size>before)batches++;
  if(found.size>=maxProducts)throw Error('سقف ایمنی تعداد محصولات اسکرولی رسید؛ فهرست کامل تأیید نشد.');
  if(limits.maxBatches&&batches>=limits.maxBatches)return [...found.values()];
