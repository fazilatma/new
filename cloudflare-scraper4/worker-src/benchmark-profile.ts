...
export function mergeBenchmarkProfile(current:any,original:any,result:any,discovered:Record<string,string>){
 ...
 if(current.extractionEngine===original.extractionEngine&&current.url===original.url&&current.pagination===original.pagination&&current.paginationValue===original.paginationValue&&JSON.stringify(current.selectors)===JSON.stringify(original.selectors)){for(const key of ['extractionEngine','extractionEngineMaster','extractionEngineMs','extractionEngineHost'])merged[key]=result[key]}
